...
try { $a = analytics_summary(7); }
catch (\Throwable $e) { json_out(['ok' => false, 'error' => 'Analytics not ready (run migrations).']); }
...
$pageLabels = ['view-main' => 'Home', 'view-cottages' => 'All cottages', 'view-experiences' => 'Experiences',
    'view-guest-bookings' => 'My stays', 'view-pay' => 'Payment', 'view-account' => 'Account'];
...
// ---- Subject + plain text ----
$subject = 'Your Blakeney week online: ' . $views . ' visit' . ($views === 1 ? '' : 's')
    . ($deltaTxt !== '' ? ' (' . $deltaTxt . ')' : '') . ', ' . $bookings . ' booking' . ($bookings === 1 ? '' : 's');

$text = "Good evening,\n\n"
    . "Here's how Cottage Holidays Blakeney did online this week.\n\n"
    . "  • Visits: {$views}" . ($deltaTxt !== '' ? " ({$deltaTxt} vs last week)" : '') . "\n"
    . "  • Unique visitors: {$uniq}\n"
    . "  • Conversion: {$convPct}% ({$bookings} booking" . ($bookings === 1 ? '' : 's') . ", {$enquiries} enquir" . ($enquiries === 1 ? 'y' : 'ies') . ")\n"
...
// ---- HTML ----
$alertHtml = ($dropPct !== null && $dropPct <= -30)
    ? email_note('<strong>Heads-up:</strong> visits are down ' . abs($dropPct) . '% on last week. Worth a look — refresh a listing photo, post an update, or check your search rankings.', '#FFA726')
    : '';

$inner = email_h('Your week online', '#D6A785')
    . email_p(email_esc(date('l j F Y')), true)
    . $alertHtml
    . email_amount('Visits this week', $views . ($deltaTxt !== '' ? ' <span style="font-size:15px;color:#a7acbb;">' . $deltaTxt . '</span>' : ''), $uniq . ' unique visitors')
    . email_rows([
        ['Conversion', $convPct . '% <span style="color:#a7acbb;">(' . $bookings . ' booking' . ($bookings === 1 ? '' : 's') . ', ' . $enquiries . ' enquir' . ($enquiries === 1 ? 'y' : 'ies') . ')</span>'],
...
json_out(['ok' => true, 'sent' => true, 'views' => $views, 'unique' => $uniq,
    'conversion_pct' => $convPct, 'bookings' => $bookings, 'drop_pct' => $dropPct, 'mail' => $res]);
