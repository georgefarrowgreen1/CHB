...
function wl_prop_name($prop) {
    try { $s = db()->prepare('SELECT name FROM properties WHERE prop_key = ?'); $s->execute([$prop]); $n = $s->fetchColumn(); if ($n) return $n; }
    catch (\Throwable $e) {}
    return $prop;
}
function wl_send($row) {
    if (empty($row['email']) || !function_exists('smtp_send')) return;
    $name = wl_prop_name($row['prop_key']);
    $dates = ($row['check_in'] && $row['check_out']) ? " for {$row['check_in']} to {$row['check_out']}" : '';
    smtp_send($row['email'], $row['name'] ?: 'there', "Good news — availability at {$name}",
...
function waitlist_notify_freed($prop, $from, $to) {
    ...
    try {
        ...
        $n = 0;
        foreach ($rows as $w) {
            try { wl_send($w); } catch (\Throwable $e) {}
            db()->prepare('UPDATE waitlist SET notified_at = NOW() WHERE id = ?')->execute([$w['id']]);
            $n++;
        }
        return $n;
...
if (basename($_SERVER['SCRIPT_NAME'] ?? '') === 'waitlist.php') {
    ...
    if ($action === 'join') {
        ...
        $ci = clean($in['check_in'] ?? ''); $co = clean($in['check_out'] ?? '');
        $note = substr(clean($in['note'] ?? ''), 0, 500);
        if ($prop === '') json_out(['error' => 'Please choose a cottage.'], 400);
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) json_out(['error' => 'Please enter a valid email address.'], 400);
        $ci = preg_match('/^\d{4}-\d{2}-\d{2}$/', $ci) ? $ci : null;
        $co = preg_match('/^\d{4}-\d{2}-\d{2}$/', $co) ? $co : null;
        if ($ci && $co && $co <= $ci) json_out(['error' => 'Check-out must be after check-in.'], 400);
        try {
            db()->prepare('INSERT INTO waitlist (prop_key, name, email, check_in, check_out, note) VALUES (?,?,?,?,?,?)')
                ->execute([$prop, $name, $email, $ci, $co, $note]);
            json_out(['ok' => true]);
        } catch (\Throwable $e) { json_out(['error' => 'Could not join the waitlist — please try again.'], 500); }
    }
    ...
    if ($action === 'notify') {
        $id = (int)($in['id'] ?? 0);
        $s = db()->prepare('SELECT * FROM waitlist WHERE id = ?'); $s->execute([$id]); $row = $s->fetch();
        if (!$row) json_out(['error' => 'Entry not found'], 404);
        try { require_once __DIR__ . '/mailer.php'; wl_send($row); } catch (\Throwable $e) {}
        db()->prepare('UPDATE waitlist SET notified_at = NOW() WHERE id = ?')->execute([$id]);
        json_out(['ok' => true]);
    }

    // default: list
    try {
        $rows = db()->query('SELECT id, prop_key, name, email, check_in, check_out, note, notified_at, created_at FROM waitlist ORDER BY created_at DESC')->fetchAll();
        json_out(['ok' => true, 'waitlist' => array_map(fn($r) => [
            'id' => (int)$r['id'], 'prop_key' => $r['prop_key'], 'name' => $r['name'], 'email' => $r['email'],
            'check_in' => $r['check_in'], 'check_out' => $r['check_out'], 'note' => $r['note'],
            'notified_at' => $r['notified_at'], 'created_at' => $r['created_at'],
        ], $rows)]);
    } catch (\Throwable $e) {
