...
try {
    ...
    json_out(['ok' => false, 'error' => 'Database unreachable: ' . $e->getMessage()], 500);
}
...
function tcheck(&$checks, $tables, $cat, $label, $needed, $okMsg)
{
    ...
    if ($missing) {
        add(
            ...
            'Run migrate.php (deploys do this automatically).',
        );
    } else {
        add($checks, $cat, $label, 'ok', $okMsg);
    }
}
...
function ccheck(&$checks, $bcols, $cat, $label, $needed, $okMsg)
{
    ...
    if ($missing) {
        add(
            ...
            'Run migrate.php to apply the latest migrations.',
        );
    } else {
        add($checks, $cat, $label, 'ok', $okMsg);
    }
...
tcheck(
    ...
    ['bookings', 'enquiries', 'guests', 'content', 'payments', 'properties'],
...
ccheck(
    ...
    ['balance_requested_at', 'balance_reminded_at'],
    'Columns present.',
);
ccheck(
    $checks,
    ...
    ['deposit_requested_at', 'deposit_reminded_at'],
    'Columns present.',
);
...
$mailOn =
    ...
    defined('SMTP_USER') &&
    SMTP_USER &&
    !in_array(SMTP_USER, $placeholderEmails, true) &&
    defined('SMTP_PASS') &&
    SMTP_PASS &&
    SMTP_PASS !== 'CHANGE_ME';
add(
    ...
    $mailOn ? 'ok' : 'warn',
    $mailOn
        ? 'Configured — try the test email.'
        : 'Not configured: confirmations, reminders, digest, newsletter & review emails won\'t send.',
    ...
    if ($ageH > 36) {
        add(
            ...
            'Last ran ' .
                ...
                'h ago (' .
                $when .
                ') — automation looks stopped. Pre-arrival emails, balance chasers, backups & re-invites are paused.',
...
if ($replyWebhook) {
    add(
        ...
        'Reply-by-email',
        'ok',
        'On (inbound route) — replies to a message alert reach the guest on the website and by email, via ' .
            REPLY_INBOX .
            '.',
    );
    ...
    if ($last) {
        $rmap = [
            'delivered' => 'delivered to the guest',
            'empty-after-strip' => 'reply was empty after trimming the quote (nothing to send)',
            'sender-not-owner' => 'came from an address not on your notification list',
            'no-thread-token' => 'was not a reply to a website message',
        ];
        $lastLine =
            ' Last reply seen (' .
            htmlspecialchars($last['from'] ?? '') .
            '): ' .
            ($rmap[$last['reason'] ?? ''] ?? ($last['reason'] ?? '')) .
            '.';
    }
    if ($err) {
        add(
            ...
            'Reply-by-email (auto)',
            'warn',
            'On, but the last mailbox check failed: ' .
                $err .
                ' (host ' .
                mailbox_pop_host() .
                '). Replies still work from the back office.',
        ...
        add(
            ...
            'Reply-by-email (auto)',
            'ok',
            'On — reply to a "new website message" email and the guest gets it on the website and by email. Reads ' .
                SMTP_USER .
                ' via ' .
                mailbox_pop_host() .
                ' (last checked ' .
...
add(
    ...
    $secretOk ? 'ok' : 'fail',
    $secretOk
        ? 'A strong secret is set.'
        : 'APP_SECRET is the default/too short — cron links, pay tokens and iCal links are guessable.',
...
add(
    ...
    'Features',
    'Welcome book',
    isset($tables['content']) ? 'ok' : 'fail',
    isset($tables['content'])
        ? 'Content storage available (edit per cottage in Preferences).'
