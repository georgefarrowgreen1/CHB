...
// Owner opt-out.
if (content_value('enquiry-nudge-off') === '1') json_out(['ok' => true, 'sent' => 0, 'off' => true]);

require_once __DIR__ . '/mailer.php';

try {
    $rows = db()->query("SELECT * FROM enquiries
        WHERE email <> '' AND nudge_sent_at IS NULL
          AND created_at <= (NOW() - INTERVAL 2 DAY)
          AND check_in >= CURDATE()")->fetchAll();
} catch (\Throwable $e) {
...
foreach ($rows as $e) {
    ...
    try {
        ...
        db()->prepare('UPDATE enquiries SET nudge_sent_at = NOW() WHERE id = ?')->execute([(int)$e['id']]);
        $sent++;
    } catch (\Throwable $ex) { /* skip this one, continue with the rest */ }
}
