...
if ($action === 'sw_notify') {
    if (!empty($_SESSION['admin_id'])) {
        ...
        json_out([
            ...
            'body' => 'You have a new notification — tap to open the back office.',
            'url' => './',
            'tag' => 'chb-owner',
        ]);
    }
...
if ($action === 'subscribe_admin' || $action === 'test_admin' || $action === 'unsubscribe_admin') {
    ...
    if ($action === 'unsubscribe_admin') {
        ...
        if ($endpoint !== '') {
            try {
                db()
                    ->prepare("DELETE FROM push_subscriptions WHERE endpoint = ? AND role = 'admin'")
                    ->execute([$endpoint]);
            } catch (\Throwable $e) {
            }
        }
...
if ($action === 'subscribe') {
    ...
    try {
        ...
        db()
            ->prepare(
                'INSERT INTO push_subscriptions (guest_id, endpoint, p256dh, auth, created_at) VALUES (?,?,?,?,NOW())',
            )
...
if ($action === 'unsubscribe') {
    ...
    if ($endpoint !== '') {
        try {
            db()
                ->prepare('DELETE FROM push_subscriptions WHERE endpoint = ? AND guest_id = ?')
                ->execute([$endpoint, $guestId]);
        } catch (\Throwable $e) {
        }
    }
    json_out(['ok' => true]);
