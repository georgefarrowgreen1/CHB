...
function pv_classify_ref($host)
{
    ...
    foreach ($engines as $needle => $name) {
        if (strpos($host, $needle) !== false) {
            return ['channel' => 'Search', 'name' => $name];
        }
    }
    $social = [
        'instagram' => 'Instagram',
    ...
    foreach ($social as $needle => $name) {
        if (strpos($host, $needle) !== false) {
            return ['channel' => 'Social', 'name' => $name];
        }
    }
...
function analytics_summary($days)
{
    ...
    try {
        db()->exec('DELETE FROM search_log WHERE created_at < (NOW() - INTERVAL 180 DAY)');
    } catch (\Throwable $e) {
    }
    ...
    $prevWindow =
        'FROM pageviews WHERE event IS NULL AND created_at >= (NOW() - INTERVAL ? DAY) AND created_at < (NOW() - INTERVAL ? DAY)';
    ...
    try {
        ...
        $exitPages = array_map(
            fn($r) => ['path' => $r['path'], 'count' => (int) $r['c']],
    ...
    $weekViews = (int) db()
        ->query('SELECT COUNT(*) FROM pageviews WHERE event IS NULL AND created_at >= (NOW() - INTERVAL 7 DAY)')
        ->fetchColumn();
    $weekUnique = (int) db()
        ->query(
            'SELECT COUNT(DISTINCT ip_hash) FROM pageviews WHERE event IS NULL AND created_at >= (NOW() - INTERVAL 7 DAY)',
        )
        ->fetchColumn();
    $prevWeek = (int) db()
        ->query(
            'SELECT COUNT(*) FROM pageviews WHERE event IS NULL AND created_at >= (NOW() - INTERVAL 14 DAY) AND created_at < (NOW() - INTERVAL 7 DAY)',
        )
        ->fetchColumn();
    ...
    try {
        foreach (
            $qDays("SELECT path, AVG(dwell_ms) a FROM pageviews
        ...
        /* dwell_ms column not migrated yet */
    }
    ...
    foreach ($allRefs as $r) {
        ...
        if ($cl['channel'] === 'Search') {
            $engines[$cl['name']] = ($engines[$cl['name']] ?? 0) + (int) $r['c'];
        }
    }
    arsort($engines);
    $channelsOut = [];
    ...
    foreach (
        $qDays(
            "SELECT event e, COUNT(*) c FROM pageviews WHERE event IS NOT NULL AND created_at >= $sinceSql GROUP BY event",
        )->fetchAll()
        as $r
    ) {
        if (isset($events[$r['e']])) {
    ...
    try {
        ...
        /* device column not migrated yet */
    }
    ...
    $bookingsN = 0;
    try {
        $enquiriesN = (int) $qDays("SELECT COUNT(*) FROM enquiries WHERE created_at >= $sinceSql")->fetchColumn();
    } catch (\Throwable $e) {
    }
    try {
    ...
    try {
        $searchDemand['total'] = (int) $qDays(
            "SELECT COUNT(*) FROM search_log WHERE created_at >= $sinceSql",
        )->fetchColumn();
        $searchDemand['noResult'] = (int) $qDays(
            "SELECT COUNT(*) FROM search_log WHERE created_at >= $sinceSql AND found = 0",
        )->fetchColumn();
        ...
        $searchDemand['topMonths'] = array_map(
            fn($r) => [
                'month' => $r['month'],
                'count' => (int) $r['c'],
                'found' => (int) $r['f'],
        ...
        $searchDemand['recentNoResult'] = array_map(
            fn($r) => [
                'mode' => $r['mode'],
                'adults' => (int) $r['adults'],
    ...
    return [
        ...
        'topPages' => array_map(
            fn($r) => ['path' => $r['path'], 'views' => (int) $r['c'], 'dwellMs' => $dwellByPath[$r['path']] ?? null],
            $pages,
        ),
