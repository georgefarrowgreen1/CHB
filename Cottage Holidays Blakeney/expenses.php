...
if ($action === 'add') {
    ...
    if ($receiptData !== '') {
        if (strlen($receiptData) > 6000) $receiptData = substr($receiptData, 0, 6000);
    ...
    try {
        db()->prepare('INSERT INTO expenses (category, description, amount, prop_key, recurring, receipt_data, expense_date) VALUES (?,?,?,?,?,?,?)')
