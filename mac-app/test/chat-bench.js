...
// ============================================================
'use strict';
const chatToolsMod = require('../src/core/chattools');
...
module.exports = { CASES, WORLD, MEMORIES, TOOL_RESULTS, benchScore, benchVerdict };

// ── THE LIVE RUNNER — only when invoked by hand. ────────────────────────────
if (require.main === module) {
    (async function () {
        ...
        // The REAL frame, byte for byte the ownerchat handler's composition.
        const sys = chatToolsMod.chatToolsIntro(WORLD.today.date)
            + '\n\n' + chatToolsMod.chatActsIntro()
            + '\n\n' + chatToolsMod.chatGroundText(WORLD, MEMORIES);
        console.log('chat-bench · ' + CASES.length + ' cases · model ' + model + ' · ' + eng.base + '\n');
        const scores = [];
        for (const spec of CASES) {
            ...
            scores.push(s);
            const mark = s.protocol && s.grounding && s.honesty ? ' ✓ ' : ' ✗ ';
            console.log(mark + spec.q + (s.why ? '  — ' + s.why : ''));
        }
        const v = benchVerdict(scores);
        console.log('\n' + v.say);
        process.exit(0);
