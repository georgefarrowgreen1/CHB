...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    foreach ($rows as $r) {
        ...
        if (strpos($r['item_key'], 'welcome-') === 0) continue;  // in-stay welcome book (may hold Wi-Fi password etc.)
        if ($r['item_key'] === 'notify-emails') continue;        // owner alert recipients — never public
...
if ($action === 'set') {
    $key = clean($in['key'] ?? '');
    if ($key === '' || strlen($key) > 190) json_out(['error' => 'Invalid key'], 400);
    ...
    if (is_private_content_key($key)) $value = encrypt_value($value);
...
if ($action === 'delete') {
    $key = clean($in['key'] ?? '');
    db()->prepare('DELETE FROM content WHERE item_key = ?')->execute([$key]);
    json_out(['ok' => true]);
}
