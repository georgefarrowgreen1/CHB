        ...
        async function oqAll() { const db = await oqDB(); return new Promise((res, rej) => { const tx = db.transaction('queue', 'readonly'); const rq = tx.objectStore('queue').getAll(); rq.onsuccess = () => res(rq.result || []); rq.onerror = () => rej(rq.error); }); }
        async function oqAdd(item) { const db = await oqDB(); await new Promise((res, rej) => { const tx = db.transaction('queue', 'readwrite'); tx.objectStore('queue').add(item); tx.oncomplete = res; tx.onerror = () => rej(tx.error); }); }
        async function oqDelete(id) { const db = await oqDB(); await new Promise((res, rej) => { const tx = db.transaction('queue', 'readwrite'); tx.objectStore('queue').delete(id); tx.oncomplete = res; tx.onerror = () => rej(tx.error); }); }
        let __oqCount = 0;
        async function oqRefreshCount() { try { __oqCount = (await oqAll()).length; } catch (e) { __oqCount = 0; } updateOnlineStatus(); }
        function updateOnlineStatus() {
            try {
                ...
                if (pill) pill.title = (offline ? 'No internet connection' : 'Reconnecting…') + (n ? ` — ${n} change${n === 1 ? '' : 's'} waiting to sync` : '');
            } catch (e) {}
        }
        // Ask the service worker to replay the queue in the background (when supported).
        async function oqRegisterSync() {
            try { const reg = await navigator.serviceWorker.ready; if (reg && 'sync' in reg) await reg.sync.register('chb-sync'); } catch (e) {}
        }
        // Send now if online; otherwise queue it and report it as queued.
        async function queueOrPost(endpoint, payload) {
            const enqueue = async () => { try { await oqAdd({ endpoint, payload, at: Date.now() }); } catch (e) {} await oqRefreshCount(); oqRegisterSync(); return { queued: true }; };
            if (navigator.onLine === false) return enqueue();
            try { return await apiPost(endpoint, payload); }
            catch (e) { if (navigator.onLine === false) return enqueue(); throw e; }
        }
        let __oqFlushing = false;
        async function oqFlush() {
            if (__oqFlushing || navigator.onLine === false) return;
            let items = []; try { items = await oqAll(); } catch (e) {}
            if (!items.length) { await oqRefreshCount(); return; }
            __oqFlushing = true;
            try {
                for (const it of items) {
                    try { await apiPost(it.endpoint, it.payload); }
                    catch (e) { if (navigator.onLine === false) break; /* online but rejected — drop it so the queue can't get stuck */ }
                    try { await oqDelete(it.id); } catch (e) {}
                }
            } finally { __oqFlushing = false; await oqRefreshCount(); }
        ...
        async function apiPost(endpoint, payload) {
            ...
            try { data = text ? JSON.parse(text) : {}; }
                ...
                throw new Error(res.ok ? 'Unexpected server response.' :
        ...
        function forceAdminLogout() {
            ...
            if (adminViews.includes(active)) { try { nav('view-main'); } catch (e) {} }
            if (!__sessionExpiredNotified) {
                __sessionExpiredNotified = true;
                try { toast('Your admin session expired — please sign in again.'); } catch (e) {}
                setTimeout(() => { __sessionExpiredNotified = false; }, 8000);
            }
        }
        ...
        function loadHeic2any() {
            ...
            __heicLoader = new Promise((resolve, reject) => {
                ...
                s.onload = () => resolve();
                s.onerror = () => { __heicLoader = null; reject(new Error('Could not load the photo converter. Please check your connection and try again.')); };
        ...
        function mapBookingFromApi(row) {
            ...
            b.priceOverride = (row.price_override != null && row.price_override !== '') ? parseFloat(row.price_override) : null;
        ...
        function nav(viewId, anchorId = null) {
            ...
            // any other page.
            try { closeGuestAuthModal(); } catch (e) {}
            try { closeGuestDetailsModal(); } catch (e) {}
            try { closeGuestSecurityModal(); } catch (e) {}
            ...
            try { if (window.setActiveTab) window.setActiveTab(viewId); } catch (e) {}

            // Keep the address bar in sync: leaving a cottage page restores the root URL.
            if (viewId !== 'view-21a') { try { clearCottageUrl(); } catch (e) {} }
            ...
            if (CUSTOMER_FACING_VIEWS.includes(viewId)) {
                try { trackView(viewId, viewId === 'view-21a' ? activeFrontProperty : ''); } catch (e) {}
            }
            ...
            if (anchorId) {
                setTimeout(() => {
                    ...
                    if (!el) return;
                    const offsetPosition = el.getBoundingClientRect().top - document.body.getBoundingClientRect().top - 90;
        ...
        async function saveContactPhone() {
            ...
            if (!dial) { glassAlert('Please enter a dial number.'); return; }
            ...
            } catch (e) { glassAlert("Couldn't save the number: " + e.message); }
        }
        ...
        function addBookingToCalendar(bookingId) {
            ...
            let propKey = null, b = null, address = '';
            const cached = guestBookingsCache.find(x => x.booking.id === bookingId);
            if (cached) { propKey = cached.propKey; b = cached.booking; address = cached.address; }
            else {
                const loc = findBookingLocation(bookingId);
                if (loc) { propKey = loc.propKey; b = dbBookings[loc.propKey][loc.idx]; }
            }
            if (!b) { glassAlert("Couldn't find that booking to add."); return; }
            ...
            const ics = [
                'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Cottage Holidays Blakeney//Booking//EN',
        ...
        function toggleTheme() {
            ...
            try { localStorage.setItem('chb-theme', isLight ? 'light' : 'dark'); } catch (e) {}
            setThemeLabel();
        }
        ...
        window.addEventListener('resize', () => {
            ...
            if (window.innerWidth > 768 && menu.classList.contains('open')) {
                ...
                document.querySelector('.menu-toggle').innerHTML = '<svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16"/></svg>';
        ...
        (function setupHeaderScroll() {
            ...
            function update() {
                ...
                if (y <= TOP_ZONE || menuOpen) {
                    ...
                    if (y > lastY) header.classList.add('header-hidden');   // scrolling down
        ...
        function lightboxNav(dir) { lightboxIndex += dir; renderLightbox(); }
        ...
        window.addEventListener('resize', () => { clearTimeout(window.__dockT); window.__dockT = setTimeout(moveDockIndicator, 120); });
        ...
        (function initCardRise() {
            document.querySelectorAll('#cottages .grid > .card').forEach(c => {
                ...
                c.addEventListener('animationend', function done(e) {
                    if (e.animationName === 'cardRise') { c.classList.remove('rise'); c.removeEventListener('animationend', done); }
                });
            });
        })();
        ...
        function renderGalleryGrid(list) {
            ...
            if (n >= 4) { grid.style.gridTemplateColumns = '2fr 1fr 1fr'; grid.style.gridTemplateRows = '1fr 1fr'; }
            else if (n === 3) { grid.style.gridTemplateColumns = '1.7fr 1fr'; grid.style.gridTemplateRows = '1fr 1fr'; }
        ...
        function loadGallerySlides(trackId) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                const near = Math.abs(i - cur) <= 1 || (cur === 0 && i === n - 1) || (cur === n - 1 && i === 0);
                if (!near) continue;
                const s = slides[i];
        ...
        function captureGeo(k) {
            ...
            if (!navigator.geolocation) { if (status) status.textContent = "This device can't share its location."; return; }
            ...
            navigator.geolocation.getCurrentPosition(async (pos) => {
                ...
                setGeoInputs(k, g);
                try { await saveContent('geo-' + k, g); } catch (e) {}
        ...
        async function saveGeoManual(k) {
            ...
            if (!isFinite(lat) || !isFinite(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
                if (status) status.textContent = 'Enter a valid latitude (−90 to 90) and longitude (−180 to 180).';
                return;
            }
            const g = { lat, lng };
            adminPrivateContent['geo-' + k] = g;
        ...
        async function openEnquiriesView() { openSettings('enquiries'); }
        ...
        async function refreshOwnerHomeBadges() {
            try {
                ...
                if (dockBadge) { dockBadge.textContent = pending; dockBadge.style.display = pending > 0 ? 'flex' : 'none'; }
            } catch (e) { /* badges are a nicety; never block the page */ }
        }
        ...
        async function openSettings(section) {
            ...
            try { refreshInboxBadge(); } catch (e) {}     // Enquiries badge
            try { loadAdminMessages(); } catch (e) {}     // Guest messages badge
            if (section) settingsOpen(section); else settingsShowIndex();
        }
        ...
        function fillHostFields() {
            const setv = (id, key) => { const e = document.getElementById(id); if (e) e.value = hostVal(key); };
            setv('host-f-name', 'host-name'); setv('host-f-badge', 'host-badge'); setv('host-f-years', 'host-years');
            setv('host-f-school', 'host-school'); setv('host-f-work', 'host-work'); setv('host-f-bio', 'host-bio');
            const photo = document.getElementById('host-edit-photo');
            if (photo) photo.style.backgroundImage = hostVal('host-photo') ? `url('${hostVal('host-photo')}')` : '';
            // Contact number now lives inside the Profile folder.
            const cp = siteContent['contact-phone'] || {};
            const dEl = document.getElementById('contact-phone-dial'); const sEl = document.getElementById('contact-phone-display');
            if (dEl) dEl.value = cp.dial || ''; if (sEl) sEl.value = cp.display || '';
        }
        // A small "row that drills into a cottage" list for accom + calendar sections.
        function cottageRowsHtml(onclickFn) {
            return Object.keys(propertyMeta).map(k =>
        ...
        async function renderAccomList() {
            const list = document.getElementById('accom-list'); const detail = document.getElementById('accom-detail');
        ...
        function settingsOpenAccomSec(k, sec) {
            ...
            if (detail) { detail.style.display = ''; detail.innerHTML = `<div class="rate-prop">${accomSectionHtml(k, sec)}</div>`; }
            const meta = ACCOM_SECTIONS.find(s => s.id === sec);
            const name = propertyMeta[k] ? propertyMeta[k].name : k;
            const title = document.getElementById('settings-panel-title');
            if (title) title.innerHTML = `${escapeHtml(name)} <span style="color:var(--text-muted);">·</span> ${meta ? meta.label : ''}`;
            settingsBackTarget = () => settingsOpenAccom(k);
            __settingsPath = { section: 'accom', prop: k, accomSec: sec };
            window.scrollTo({ top: 0, behavior: 'smooth' });
        }
        function renderCalendarList() {
            const list = document.getElementById('calendar-list'); const detail = document.getElementById('calendar-detail');
            if (detail) { detail.style.display = 'none'; detail.innerHTML = ''; }
            if (list) { list.style.display = ''; list.innerHTML = `<div class="settings-group">${cottageRowsHtml('settingsOpenCalendar')}</div>`; }
            settingsBackTarget = () => settingsShowIndex();
            const title = document.getElementById('settings-panel-title'); if (title) title.textContent = SETTINGS_TITLES.calendar;
        }
        ...
        const CANCELLATION_POLICIES = {
            flexible: { name: 'Flexible', points: ['Full refund at least 1 day before check-in', 'Partial refund within 1 day of check-in'] },
            moderate: { name: 'Moderate', points: ['Full refund at least 5 days before check-in', 'Partial refund within 5 days of check-in'] },
            limited:  { name: 'Limited',  points: ['Full refund at least 14 days before check-in', 'Partial refund 7–14 days before check-in'] },
        };
        ...
        function renderCancelList() {
            const list = document.getElementById('cancel-list'); const detail = document.getElementById('cancel-detail');
            if (detail) { detail.style.display = 'none'; detail.innerHTML = ''; }
            if (list) { list.style.display = ''; list.innerHTML = `<div class="settings-group">${cancelRowsHtml()}</div>`; }
            settingsBackTarget = () => settingsShowIndex();
            const title = document.getElementById('settings-panel-title'); if (title) title.textContent = SETTINGS_TITLES.cancel;
        }
        ...
        function cancelPickerHtml(propKey) {
            ...
            const cards = Object.keys(CANCELLATION_POLICIES).map(pk => {
        ...
        function settingsOpenCancel(propKey) {
            const list = document.getElementById('cancel-list'); const detail = document.getElementById('cancel-detail');
            if (list) list.style.display = 'none';
            if (detail) { detail.style.display = ''; detail.innerHTML = cancelPickerHtml(propKey); }
            const title = document.getElementById('settings-panel-title'); if (title) title.textContent = (propertyMeta[propKey] ? propertyMeta[propKey].name : propKey);
        ...
        function setCancelPolicy(propKey, polKey) {
            ...
            try { localStorage.setItem(`${propKey}-cancellation-policy`, polKey); } catch (e) {}
        ...
        async function openReviews() { openSettings('reviews'); }
        ...
        async function loadCalendarSyncProp(key) {
            ...
            catch (e) { box.innerHTML = `<p style="color:#E53935;">${escapeHtml(e.message)}</p>`; return; }
        ...
        async function saveSyncFeeds(key, quiet) {
            ...
            } catch (e) { if (!quiet) glassAlert("Couldn't save: " + e.message); }
        }
        async function runSync(key) {
            try {
                ...
                if (document.getElementById('calendar-detail') && document.getElementById('calendar-detail').style.display !== 'none') loadCalendarSyncProp(key);
                else loadCalendarSync();
            } catch (e) { glassAlert("Sync failed: " + e.message); }
        }
        ...
        async function loadGuestList() {
            ...
            if (guests.length === 0) {
                box.innerHTML = '<p style="color:var(--text-muted);font-size:0.85rem;">No guest accounts yet.</p>';
                return;
            }
            box.innerHTML = `
        ...
        async function resetGuestPassword(email) {
            if (!isAuthenticated) { tryAccessBackOffice(); return; }
            const next = await glassPrompt(`Set a NEW password for ${email}\n\n(at least 4 characters — you'll tell the guest this):`, '', { password: true });
            if (next === null) return;
            if (next.trim().length < 4) { glassAlert("Password must be at least 4 characters."); return; }
            try {
                await apiPost('auth.php', { action: 'guest_reset_password', email, next });
                glassAlert(`Password reset for ${email}.\n\nGive them the new password and ask them to log in and change it.`);
            } catch (e) {
        ...
        async function changeAdminPassword() {
            ...
            const next = await glassPrompt("Enter a NEW password (at least 4 characters):", '', { password: true });
            if (next === null) return;
            if (next.trim().length < 4) { glassAlert("Password must be at least 4 characters."); return; }
            const confirmNext = await glassPrompt("Re-enter the NEW password to confirm:", '', { password: true });
            if (confirmNext === null) return;
            if (confirmNext !== next) { glassAlert("The new passwords don't match. Nothing was changed."); return; }
            try {
        ...
        async function openAccounts() {
            ...
            try {
                ...
                years = res.years || [];
            } catch (e) { glassAlert("Couldn't load accounts: " + e.message); return; }
            ...
            try { await loadDepositReturns(); } catch (e) {}
            try { renderDepositsDue(); } catch (e) {}
            try { renderMoneyPanel(); } catch (e) {}
            try { renderMoneyForecast(); } catch (e) {}
            try { renderMoneyFeed(); } catch (e) {}
            try { await loadExpenses(); } catch (e) {}
            await renderAccounts();
            try { renderMoneyOverview(); } catch (e) {}
            accountsShowIndex();
        }

        // ---- Money router: Apple-style index → drill-down sub-pages (mirrors Settings) ----
        const ACCOUNTS_TITLES = { payments: 'Payments & balances', recent: 'Recent payments', income: 'Income & tax', expenses: 'Expenses' };
        let allExpenses = [];   // cached expense rows (client buckets by tax year)
        async function loadExpenses() {
            try { const r = await apiGet('expenses.php'); allExpenses = Array.isArray(r.expenses) ? r.expenses : []; }
            catch (e) { allExpenses = []; }
        }
        ...
        function accountsOpen(section) {
            ...
            try {
                if (section === 'payments') { renderDepositsDue(); renderMoneyPanel(); }
                else if (section === 'recent') { renderMoneyFeed(); }
                else if (section === 'income') { renderMoneyForecast(); renderAccounts(); }
                else if (section === 'expenses') { renderExpenses(); }
            } catch (e) {}
            window.scrollTo({ top: 0, behavior: 'smooth' });
        }
        function accountsBack() { accountsShowIndex(); }
        ...
        async function renderAccounts() {
            ...
            } catch (e) { content.innerHTML = `<div class="accounts-empty">Couldn't load: ${escapeHtml(e.message)}</div>`; return; }
            ...
            const qRows = qBounds.map(([lbl, s, e]) => {
                const inc = payments.filter(p => (p.payment_date || '') >= s && (p.payment_date || '') <= e).reduce((a, p) => a + (p.income_part || 0), 0);
                const exp = expYear.filter(x => (x.date || '') >= s && (x.date || '') <= e).reduce((a, x) => a + (x.amount || 0), 0);
        ...
        const EXPENSE_CATS = ['Cleaning', 'Laundry', 'Maintenance', 'Utilities', 'Supplies', 'Insurance', 'Fees', 'Marketing', 'Other'];
        ...
        function pickExpenseReceipt() {
            ...
            input.onchange = async () => {
                ...
                if (prev) prev.innerHTML = '<span id="exp-scan-status" style="font-size:0.75rem;color:var(--text-muted);">Reading…</span>';
                try {
                    let img = file;
                    if (isHeic(file)) { try { img = await ensureUploadable(file); } catch (e) {} } // HEIC → JPEG so OCR can read it
                    await scanReceiptFile(img);
                } catch (e) {
                    const st = document.getElementById('exp-scan-status'); if (st) st.textContent = 'couldn’t read it — enter manually';
                }
            };
            input.click();
        }
        ...
        function loadTesseract() {
            ...
            __tessLoading = new Promise((resolve, reject) => {
                ...
                s.onload = resolve;
                s.onerror = () => { __tessLoading = null; reject(new Error('scanner unavailable')); };
                document.head.appendChild(s);
            });
        ...
        async function scanReceiptFile(file) {
            ...
            try {
                ...
                if (status()) status().textContent = filled ? '✓ read — check the details' : 'couldn’t read it — enter manually';
            } catch (e) {
        ...
        function parseReceiptDate(text) {
            ...
            const ok = (y, m, d) => (m >= 1 && m <= 12 && d >= 1 && d <= 31) ? `${y}-${pad(m)}-${pad(d)}` : '';
            let m;
            ...
            if (m && MON[m[2].slice(0,3).toLowerCase()]) return ok(yr(+m[3]), MON[m[2].slice(0,3).toLowerCase()], +m[1]);
            m = text.match(/([A-Za-z]{3,})\.?\s*(\d{1,2})\s*,?\s*(\d{2,4})/);            // Jun 16, 2026
            if (m && MON[m[1].slice(0,3).toLowerCase()]) return ok(yr(+m[3]), MON[m[1].slice(0,3).toLowerCase()], +m[2]);
            return '';
        }
        // Pull line items (name + price) out of OCR text, skipping totals/payment lines.
        function parseReceiptItems(lines) {
            const skip = /(sub-?total|^total|vat|tax|change|cash|card|balance|amount due|to pay|visa|master|debit|credit|tip|service charge|rounding|invoice|receipt|tel\b|phone|www\.|http|@|^date|^time)/i;
            ...
            for (const l of lines) {
                ...
                if (m) {
                    const name = m[1].replace(/\s{2,}/g, ' ').replace(/[.\s]+$/, '').trim();
                    if (name && /[a-z]/i.test(name)) items.push({ name: name.slice(0, 42), price: parseFloat(m[2].replace(',', '.')) });
                }
        ...
        function applyReceiptText(text) {
            if (!text || !text.trim()) { renderReceiptCard(null); return false; }
            const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
            const moneyRe = /(?:£|gbp|\$)?\s*(\d{1,5}[.,]\d{2})\b/i;
            // Amount: prefer a "total"-ish line; else the largest money figure on the receipt.
            let amount = null;
            const totalLine = lines.find(l => /total|amount due|balance due|to pay|grand total/i.test(l) && moneyRe.test(l));
            if (totalLine) { const mm = totalLine.match(moneyRe); if (mm) amount = parseFloat(mm[1].replace(',', '.')); }
            if (amount == null) {
                let max = 0; const re = /(\d{1,5}[.,]\d{2})\b/g; let mm;
                lines.forEach(l => { while ((mm = re.exec(l))) { const v = parseFloat(mm[1].replace(',', '.')); if (v > max) max = v; } });
                if (max > 0) amount = max;
            }
            const date = parseReceiptDate(text);
            const supplier = (lines.find(l => /[a-z]/i.test(l) && l.replace(/[^a-z]/gi, '').length >= 3) || '').slice(0, 42);
            const items = parseReceiptItems(lines);
            __lastReceipt = { supplier: supplier || '', date: date || '', items, amount: (amount != null ? amount : null) };
        ...
        function receiptCardHtml(r) {
            ...
            const items = (r.items || []).map(it =>
                `<div style="display:flex;justify-content:space-between;gap:12px;font-size:0.82rem;"><span>${escapeHtml(it.name)}</span><span style="color:var(--text-muted);">${gbp(it.price)}</span></div>`).join('');
        ...
        function renderReceiptCard(r) {
            ...
            el.innerHTML = empty ? '' : `<div class="mo-card" style="max-width:420px;margin-top:10px;"><div class="mo-card-title">Scanned receipt</div>${receiptCardHtml(r)}</div>`;
        }
        ...
        function renderExpenses() {
            ...
            const cottageOpts = ['<option value="">All / general</option>']
                .concat(Object.keys(propertyMeta).map(k => `<option value="${k}">${escapeHtml(propertyMeta[k].name)}</option>`)).join('');
            const rowsByYear = {};
            allExpenses.forEach(x => { const y = taxYearStartOf(x.date); (rowsByYear[y] = rowsByYear[y] || []).push(x); });
            const years = Object.keys(rowsByYear).sort((a, b) => b - a);
            // Parse stored receipt data (supplier/date/items/amount) for the list toggles.
            __expenseReceipts = {};
            allExpenses.forEach(x => { if (x.receipt_data) { try { __expenseReceipts[x.id] = JSON.parse(x.receipt_data); } catch (e) {} } });
            ...
            if (years.length) {
                ...
                const byCat = {};
                items.forEach(x => { byCat[x.category || 'Other'] = (byCat[x.category || 'Other'] || 0) + (x.amount || 0); });
                const max = Math.max(1, ...Object.values(byCat));
                const bars = osHBars(Object.keys(byCat).sort((a, b) => byCat[b] - byCat[a]).map(c => ({ label: c, value: Math.round(byCat[c]), max, valLabel: gbp(byCat[c]), color: 'var(--accent)' })));
            ...
            const list = years.length ? years.map(y => {
            ...
            }).join('') : `<p style="font-size:0.85rem;color:var(--text-muted);margin-top:14px;">No expenses logged yet.</p>`;
        ...
        function editExpense(id) {
            ...
            const cat = document.getElementById('exp-cat'); if (cat) cat.value = x.category || 'General';
            set('exp-amount', x.amount != null ? Number(x.amount).toFixed(2) : '');
            const prop = document.getElementById('exp-prop'); if (prop) prop.value = x.prop_key || '';
            set('exp-desc', x.description || '');
            const rec = document.getElementById('exp-recurring'); if (rec) rec.checked = !!x.recurring;
            try { __lastReceipt = x.receipt_data ? JSON.parse(x.receipt_data) : null; } catch (e) { __lastReceipt = null; }
            renderReceiptCard(__lastReceipt);
            const addBtn = document.querySelector('.exp-add-btn'); if (addBtn) addBtn.textContent = 'Update';
            const form = document.querySelector('.exp-add-form'); if (form) form.scrollIntoView({ behavior: 'smooth', block: 'center' });
            const amt = document.getElementById('exp-amount'); if (amt) amt.focus();
        }
        async function addExpense() {
            ...
            const payload = { action: editing ? 'update' : 'add', date: v('exp-date'), category: v('exp-cat'), amount: parseFloat(v('exp-amount')) || 0, prop: v('exp-prop'), description: v('exp-desc'), recurring };
            if (editing) payload.id = editing;
            // Attach the scanned receipt data (supplier/date/items/amount) if present.
            if (__lastReceipt && ((__lastReceipt.items || []).length || __lastReceipt.supplier)) payload.receipt_data = JSON.stringify(__lastReceipt);
            if (!payload.amount || payload.amount <= 0) { glassAlert('Enter an amount greater than zero.'); return; }
            try {
                const res = await queueOrPost('expenses.php', payload);
                __editingExpenseId = null;
                __lastReceipt = null;
                const prev = document.getElementById('exp-receipt-prev'); if (prev) prev.innerHTML = '';
                const card = document.getElementById('exp-receipt-card'); if (card) card.innerHTML = '';
                if (res && res.queued) { try { clearExpenseForm(); } catch (e) {} toast('Saved offline — it’ll sync when you reconnect.'); return; }
                await loadExpenses();
                renderExpenses();
        ...
        function clearExpenseForm() {
            ...
            const cat = document.getElementById('exp-cat'); if (cat) cat.selectedIndex = 0;
            set('exp-amount', '');
            const prop = document.getElementById('exp-prop'); if (prop) prop.value = '';
            set('exp-desc', '');
            const rec = document.getElementById('exp-recurring'); if (rec) rec.checked = false;
            __lastReceipt = null;
            __editingExpenseId = null;
            const addBtn = document.querySelector('.exp-add-btn'); if (addBtn) addBtn.textContent = 'Add';
            const prev = document.getElementById('exp-receipt-prev'); if (prev) prev.innerHTML = '';
            const card = document.getElementById('exp-receipt-card'); if (card) card.innerHTML = '';
        }
        ...
        async function repeatExpense(id) {
            ...
            if (!await glassConfirm(`Add a copy of "${x.category}${x.description ? ' · ' + x.description : ''}" (${gbp(x.amount)}) dated ${nd}?`)) return;
            try {
                await apiPost('expenses.php', { action: 'add', date: nd, category: x.category, amount: x.amount, prop: x.prop_key || '', description: x.description || '', recurring: 1 });
                await loadExpenses(); renderExpenses();
                try { renderAccounts(); } catch (e) {}
                try { renderMoneyOverview(); } catch (e) {}
                toast('Recurring expense added for next month.');
            } catch (e) { glassAlert("Couldn't add: " + e.message); }
        }
        async function deleteExpense(id) {
            ...
            try {
                ...
                try { renderAccounts(); } catch (e) {}
                try { renderMoneyOverview(); } catch (e) {}
            } catch (e) { glassAlert("Couldn't remove: " + e.message); }
        }
        ...
        function renderDepositsDue() {
            ...
            const items = rows.map(({ propKey, b, dh }) => `
        ...
        async function returnDeposit(bookingId) {
            ...
            if (dh.held <= 0) { glassAlert('No damage deposit is being held for this booking.'); return; }
            const entered = await glassPrompt(`Amount to return (£). Held: ${gbp(dh.held)}. Enter less to retain some for damage:`, String(dh.held));
            if (entered === null) return;
            const amount = Math.round((parseFloat(entered) || 0) * 100) / 100;
            if (!(amount > 0 && amount <= dh.held + 0.001)) { glassAlert(`Enter an amount between £0 and ${gbp(dh.held)}.`); return; }
            let note = '';
            if (amount < dh.held - 0.001) {
                const r = await glassPrompt('Reason for retaining the rest (shown to the guest), e.g. "broken lamp":', '');
                if (r === null) return;
                note = r.trim();
            }
            if (!await glassConfirm(`Return ${gbp(amount)} of the damage deposit to ${booking.name}?`)) return;
            try {
            ...
            } catch (e) { glassAlert("Couldn't return the deposit: " + e.message); }
        }
        ...
        async function cancelBooking(bookingId) {
            ...
            const entered = await glassPrompt(`Cancel this booking. Refund amount (£) to the guest — 0 for none. Received so far: ${gbp(ps.deposit)}:`, String(ps.deposit || 0));
            if (entered === null) return;
            const refund = Math.round((parseFloat(entered) || 0) * 100) / 100;
            if (refund < 0) { glassAlert('Refund cannot be negative.'); return; }
            const reason = await glassPrompt('Reason for cancellation (optional, shown to the guest):', '');
            if (reason === null) return;
            if (!await glassConfirm(`Cancel ${booking.name}'s booking${refund > 0 ? ` and refund ${gbp(refund)}` : ''}? This frees the dates and emails the guest.`)) return;
            try {
                const r = await apiPost('bookings.php', { action: 'cancel', id: booking.dbId, refund_amount: refund, reason: reason.trim() });
                toast('Booking cancelled.' + (r.manual_refund ? " Couldn't auto-refund — please refund manually." : ''));
                try { closeDetailsModal(); } catch (e) {}
                await loadData(); renderCalendar();
                if (document.getElementById('view-accounts') && document.getElementById('view-accounts').classList.contains('active')) afterPaymentChange(bookingId);
            } catch (e) { glassAlert("Couldn't cancel: " + e.message); }
        }
        ...
        function renderMoneyOverview() {
            ...
            for (let i = 11; i >= 0; i--) {
                ...
                months.push({ key: d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0'), short: d.toLocaleDateString('en-GB', { month: 'short' }), received: 0 });
            }
            const monthIndex = {}; months.forEach((m, i) => monthIndex[m.key] = i);

            let monthRevenue = 0, receivedTY = 0, owedUpcoming = 0, receivedUpcoming = 0, owedCount = 0, next90 = 0;
            const byCottageTY = {}; Object.keys(propertyMeta).forEach(k => byCottageTY[k] = 0);
            ...
            Object.keys(dbBookings).forEach(propKey => {
                (dbBookings[propKey] || []).forEach(b => {
                    ...
                    if (recv > 0 && b.paymentDate) {
                        ...
                        if (taxYearStartOf(b.paymentDate) === curTY) { receivedTY += recv; byCottageTY[propKey] = (byCottageTY[propKey] || 0) + recv; }
                    }
                    if ((b.checkIn || '') >= monthStart && (b.checkIn || '') <= monthEnd) monthRevenue += ps.total || 0;
                    if ((b.checkOut || '') >= today) {
                        receivedUpcoming += ps.deposit || 0;
                        if (!ps.fullyPaid) { owedUpcoming += ps.balance || 0; owedCount++; }
                        if ((b.checkIn || '') >= today && (b.checkIn || '') <= in90) next90 += ps.total || 0;
                    }
                });
            });
            ...
            const cottageBars = osHBars(Object.keys(byCottageTY).map(k => ({ label: propertyMeta[k].name, value: Math.round(byCottageTY[k]), max: cottageMax, valLabel: gbp(byCottageTY[k]), color: `var(--prop-${k})` })));
            const chase = owedUpcoming > 0.5 ? `<div class="mo-chase">
        ...
        function renderMoneyPanel() {
            ...
            Object.keys(dbBookings).forEach(propKey => {
                (dbBookings[propKey] || []).forEach(b => {
                    if ((b.checkOut || '') >= today) rows.push({ propKey, b, ps: paymentSummary(propKey, b) });
                });
            });
            // "Owed to you" focus: unpaid/part-paid first (an action queue), then settled;
            // within each group, soonest check-in first.
            rows.sort((a, b) => {
                const ap = a.ps.fullyPaid ? 1 : 0, bp = b.ps.fullyPaid ? 1 : 0;
            ...
            const collectedPct = (receivedTotal + owedTotal) > 0 ? Math.round(receivedTotal / (receivedTotal + owedTotal) * 100) : (receivedTotal > 0 ? 100 : 0);
            ...
            const cards = rows.map(({ propKey, b, ps }) => {
                const meta = paymentMeta[b.payment] || { label: '—', dot: '#888' };
                const ci = dpParse(b.checkIn), t0 = dpParse(today);
                ...
                const sqBtns = (squareAdminEnabled && b.email) ? `
                ...
                const history = (squareAdminEnabled && b.email)
                ...
                const dh = damageHeld(propKey, b);
                const depLine = dh.collected > 0 ? `<div class="money-deposit">
                        <span>Refundable damage deposit: ${dh.held > 0
                ...
                <div class="money-row glass-panel${dueSoon ? ' due-soon' : ''}">
                    ...
                    <div class="money-actions">
                        ...
                        <select class="input-glass field-sm" onchange="updatePaymentStatus('${b.id}', this.value)" title="Payment status">
                            ${Object.keys(paymentMeta).map(k => `<option value="${k}" ${b.payment === k ? 'selected' : ''}>${paymentMeta[k].label}</option>`).join('')}
                        </select>
            ...
            if (squareAdminEnabled) rows.forEach(({ b }) => { if (b.email) loadBookingPayments(b.id); });
        }
        ...
        async function renderMoneyFeed() {
            ...
            if (!el) return;
            let list = [];
            try { const r = await apiPost('bookings.php', { action: 'recent_payments' }); list = r.payments || []; }
            ...
            let grossIn = 0, feeSum = 0, feeKnown = 0;
            const rows = list.map(p => {
                const isReturn = (p.kind === 'refund' || p.kind === 'damages_return');
                const label = p.kind === 'refund' ? 'Refund'
                    : (p.kind === 'damages_return' ? 'Deposit return' : (p.kind.charAt(0).toUpperCase() + p.kind.slice(1)));
                ...
                if (!isReturn) { grossIn += gross; if (fee != null) { feeSum += fee; feeKnown++; } }
                // Gross / fee / net per card-in transaction (fees settle after the charge).
                const feeNote = (!isReturn && fee != null && fee > 0) ? ` · fee ${gbp(fee)} · net ${gbp(Math.max(0, gross - fee))}` : '';
                ...
                const who = (p.name || 'Guest') + (deleted ? ' · deleted booking' : '') + (note ? ' · ' + note : '');
            ...
            const recon = grossIn > 0 ? `<div class="mo-card" style="margin:-2px 0 12px;">
        ...
        function renderMoneyForecast() {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                months.push({
                    ...
                    revenue: 0, bookings: 0, nights: new Set()
                });
            }
            ...
            const addNights = (m, propKey, checkIn, checkOut) => {
                let d = dpParse(checkIn), end = dpParse(checkOut);
                if (!d || !end) return;
            ...
            Object.keys(dbBlocks || {}).forEach(propKey => {
                (dbBlocks[propKey] || []).forEach(bl => months.forEach(m => addNights(m, propKey, bl.checkIn, bl.checkOut)));
            });
            ...
            const chart = osVBars(months.map(m => ({ short: m.label.split(' ')[0].slice(0, 3), label: m.label, value: m.revenue })), moneyShort);
        ...
        function exportAccountsCSV() {
            ...
            const payments = (accountsReport.payments || []).slice()
                .sort((a, b) => (a.payment_date || '').localeCompare(b.payment_date || ''));
            const esc = (v) => `"${String(v == null ? '' : v).replace(/"/g, '""')}"`;
            let csv = 'Date,Booking Ref,Guest,Property,Method,Rental Income (GBP),Held Deposit (GBP),Received (GBP)\n';
            payments.forEach(r => {
                csv += [esc(r.payment_date), esc(bookingRef('b' + r.id)), esc(r.name || ''),
                        esc(r.property_name || ''), esc(r.payment_method || ''),
                        (parseFloat(r.income_part) || 0).toFixed(2),
                        (parseFloat(r.held_part) || 0).toFixed(2),
                        (parseFloat(r.received) || 0).toFixed(2)].join(',') + '\n';
            });
            const inc = (accountsReport.total || 0), held = (accountsReport.held_deposits || 0);
            csv += `,,,,Totals,${inc.toFixed(2)},${held.toFixed(2)},${(inc + held).toFixed(2)}\n`;
            // Expenses for the same tax year + a net-profit summary.
            const exp = expensesForYear(startYear).slice().sort((a, b) => (a.date || '').localeCompare(b.date || ''));
            const expTotal = exp.reduce((s, x) => s + (x.amount || 0), 0);
            csv += '\nExpenses\nDate,Category,Note,Cottage,Amount (GBP)\n';
            exp.forEach(x => {
                csv += [esc(x.date), esc(x.category || ''), esc(x.description || ''),
                        esc(x.prop_key && propertyMeta[x.prop_key] ? propertyMeta[x.prop_key].name : (x.prop_key || '')),
                        (x.amount || 0).toFixed(2)].join(',') + '\n';
            });
        ...
        function downloadYearStatement(startYear) {
            if (!window.jspdf || !window.jspdf.jsPDF) { glassAlert('The PDF tool is still loading — please try again in a moment.'); return; }
            ...
            const left = 50, right = W - 50;
            let y = 60;
            const line = (yy) => { doc.setDrawColor(210); doc.line(left, yy, right, yy); };
            const rowLR = (l, rr, yy, bold) => { doc.setFont('helvetica', bold ? 'bold' : 'normal'); doc.text(String(l), left, yy); doc.text(String(rr), right, yy, { align: 'right' }); };
            const brk = () => { if (y > H - 70) { doc.addPage(); y = 60; } };
            ...
            doc.setTextColor(0); y += 16; doc.setFontSize(9); doc.setTextColor(120);
            doc.text(`UK tax year ${taxYearLabel(startYear)}`, left, y);
            doc.setTextColor(0); y += 24; line(y); y += 28;

            doc.setFont('helvetica', 'bold'); doc.setFontSize(12); doc.text('Summary', left, y); y += 20; doc.setFontSize(10);
            rowLR('Rental income received', gbp(income), y); y += 18;
            rowLR('Expenses', '− ' + gbp(expTotal), y); y += 18;
            y += 4; line(y); y += 18;
            rowLR('Net profit', gbp(net), y, true); y += 22;
            if (held > 0) { doc.setFontSize(9); doc.setTextColor(120); doc.text(`(${gbp(held)} refundable damage deposits held — not income)`, left, y); doc.setTextColor(0); doc.setFontSize(10); y += 18; }
            y += 8; line(y); y += 28;

            doc.setFont('helvetica', 'bold'); doc.setFontSize(12); doc.text('Income by cottage', left, y); y += 20; doc.setFontSize(10);
            Object.keys(byProp).forEach(k => { rowLR(propertyMeta[k] ? propertyMeta[k].name : k, gbp(byProp[k]), y); y += 18; brk(); });
            y += 10; line(y); y += 28;

            doc.setFont('helvetica', 'bold'); doc.setFontSize(12); doc.text('Expenses', left, y); y += 20; doc.setFontSize(9);
            if (!exp.length) { doc.setTextColor(120); doc.text('No expenses logged.', left, y); doc.setTextColor(0); y += 18; }
            exp.forEach(x => {
                const lbl = `${x.date}  ${x.category || ''}${x.description ? ' · ' + x.description : ''}`;
                rowLR(lbl.length > 70 ? lbl.slice(0, 70) + '…' : lbl, gbp(x.amount), y); y += 16; brk();
            });
            y += 4; line(y); y += 18; doc.setFontSize(10);
            rowLR('Total expenses', gbp(expTotal), y, true); y += 28;

            doc.setFontSize(8); doc.setTextColor(120);
            doc.text('A record-keeping aid, not formal accounting advice. Income is allocated to the tax year by each payment date.', left, y);
        ...
        async function saveGuestProfile() {
            ...
            const show = (t, ok) => { if (msg) { msg.textContent = t; msg.style.color = ok ? '#4CAF50' : '#E53935'; msg.style.display = 'block'; } };
            if (!address) { show('Please enter your UK address.', false); return; }
            if (!isUkPostcode(postcode)) { show('Please enter a valid UK postcode.', false); return; }
            try {
                const res = await apiPost('auth.php', { action: 'guest_update_profile', phone, address, postcode });
                currentGuest = res.guest || currentGuest;
                show('Saved.', true);
            } catch (e) { show("Couldn't save: " + e.message, false); }
        }
        ...
        function openGuestDetailsModal() {
            ...
            try { if (window.setGuestDockOverlay) window.setGuestDockOverlay('account'); } catch (e) {}
        }
        function closeGuestDetailsModal() {
            const m = document.getElementById('guest-details-modal');
            if (!m || !m.classList.contains('open')) return;
            m.classList.add('closing');
            try { if (window.setGuestDockOverlay) window.setGuestDockOverlay(null); } catch (e) {}
        ...
        function openGuestSecurityModal() {
            ...
            const msg = document.getElementById('pw-msg'); if (msg) msg.style.display = 'none';
            loadPasskeys();
        ...
        async function changeGuestPassword() {
            ...
            const show = (t, ok) => { if (msg) { msg.textContent = t; msg.style.color = ok ? '#4CAF50' : '#E53935'; msg.style.display = 'block'; } };
            if (!cur || !nw) { show('Please fill in your current and new password.', false); return; }
            if (nw.length < 4) { show('Your new password must be at least 4 characters.', false); return; }
            if (nw !== cf) { show('The new passwords do not match.', false); return; }
            try {
        ...
        async function openGuestArea() {
            ...
            if (currentGuest) {
                closeGuestAuthModal(); nav('view-guest-bookings');
        ...
        async function guestRegister() {
            ...
            const showErr = (m) => { err.innerText = m; err.style.display = 'block'; };
            ...
            if (!address) { showErr('Please enter your UK address.'); return; }
            if (!isUkPostcode(postcode)) { showErr('Please enter a valid UK postcode.'); return; }
            if (password.length < 4) { showErr('Please choose a password of at least 4 characters.'); return; }

            try {
                const res = await apiPost('auth.php', { action: 'guest_register', name, email, phone, address, postcode, password });
                currentGuest = res.guest;
                isAuthenticated = false; setAuthUI();   // one role at a time: drop any admin session
        ...
        async function guestLogin() {
            ...
            const showErr = (m) => { err.innerText = m; err.style.display = 'block'; };
            if (!id || !password) { showErr('Please enter your email/username and password.'); return; }
            ...
            try {
                ...
                isAuthenticated = true; setAuthUI();
                currentGuest = null; setGuestUI();        // one role at a time
                err.style.display = 'none';
                closeGuestAuthModal();
                nav('view-backoffice'); refreshOwnerHomeBadges();
                return;
            } catch (adminErr) { /* not the owner — try a guest login */ }
        ...
        async function requestMagicLink() {
            ...
            const show = (t, ok) => { if (msg) { msg.textContent = t; msg.style.color = ok ? '#4CAF50' : '#E53935'; msg.style.display = 'block'; } };
            if (!email || email.indexOf('@') < 1) { show('Enter your email above first, then tap this.', false); return; }
            try {
                await apiPost('auth.php', { action: 'guest_magic_request', email });
                show('Check your inbox — if that email has an account, a sign-in link is on its way. It expires in 30 minutes.', true);
            } catch (e) { show("Couldn't send the link — please try again.", false); }
        }
        ...
        async function maybeConsumeMagicLink() {
            let usp;
            try { usp = new URLSearchParams(window.location.search); } catch (e) { return false; }
            const gid = parseInt(usp.get('mlogin'), 10);
            const ts = parseInt(usp.get('t'), 10);
            const token = usp.get('k') || '';
            if (!gid || !ts || !token) return false;
            const clean = () => { try { history.replaceState(null, '', window.location.pathname); } catch (e) {} };
            try {
                const res = await apiPost('auth.php', { action: 'guest_magic_consume', guest_id: gid, ts, token });
                currentGuest = res.guest;
                ...
                await renderGuestBookings();
                try { toast('Signed in — welcome back!'); } catch (e) {}
            } catch (e) {
                clean();
                try { glassAlert(e.message || 'That sign-in link could not be used. Please request a new one.'); } catch (e2) {}
            }
            return true;
        }
        ...
        function prepCreateOptions(o) {
            ...
            if (o.excludeCredentials) o.excludeCredentials.forEach(c => { if (c.id) c.id = b64urlToBuf(c.id); });
            return o;
        }
        function prepGetOptions(o) {
            if (o.challenge) o.challenge = b64urlToBuf(o.challenge);
            if (o.allowCredentials) o.allowCredentials.forEach(c => { if (c.id) c.id = b64urlToBuf(c.id); });
            return o;
        }
        ...
        async function addPasskey() {
            if (!passkeysSupported()) { glassAlert("This device or browser doesn't support passkeys."); return; }
            try {
        ...
        async function passkeyLogin() {
            ...
            try {
                ...
                if (e && e.name === 'NotAllowedError') return; // user cancelled
                const err = document.getElementById('login-error');
                if (err) { err.innerText = "Passkey sign-in failed: " + (e.message || e); err.style.display = 'block'; }
            }
        }
        ...
        async function loadPasskeys() {
            ...
            try {
                ...
                if (keys.length === 0) { box.innerHTML = '<p style="font-size:0.82rem;color:var(--text-muted);">No passkeys yet.</p>'; return; }
                box.innerHTML = keys.map(k => `<div style="display:flex;justify-content:space-between;align-items:center;border:1px solid var(--glass-border);border-radius:10px;padding:10px 14px;margin-bottom:8px;">
        ...
        async function deletePasskey(id) {
            ...
            try { await apiPost('passkeys.php', { action: 'delete', id }); loadPasskeys(); }
            catch (e) { glassAlert("Couldn't remove: " + e.message); }
        }
        ...
        async function loadAdminPasskeys() {
            ...
            try {
                ...
                const keys = res.passkeys || [];
                if (keys.length === 0) { box.innerHTML = '<p style="font-size:0.82rem;color:var(--text-muted);">No passkeys yet. Your password is still your way in.</p>'; return; }
                box.innerHTML = keys.map(k => `<div style="display:flex;justify-content:space-between;align-items:center;border:1px solid var(--glass-border);border-radius:10px;padding:10px 14px;margin-bottom:8px;">
                    <span style="font-size:0.88rem;">${escapeHtml(k.label || 'Passkey')}<span style="color:var(--text-muted);font-size:0.75rem;"> · added ${(k.created_at||'').split(' ')[0]}</span></span>
        ...
        async function deleteAdminPasskey(id) {
            if (!await glassConfirm('Remove this passkey? You can still sign in with your password.')) return;
            try { await apiPost('passkeys.php', { action: 'admin_delete', id }); loadAdminPasskeys(); }
            catch (e) { glassAlert("Couldn't remove: " + e.message); }
        }
        ...
        async function renderGuestBookings() {
            ...
            if (!currentGuest) { openGuestArea(); return; }
            if (welcome) welcome.innerText = `Welcome back, ${currentGuest.name.split(' ')[0]} — here are your stays.`;
            loadPasskeys();
            fillGuestProfile();

            // Fetch this guest's own bookings + pending enquiries (incl. property address)
            let rows = [], enqRows = [], completedStays = 0;
            try {
            ...
            const pendingMine = enqRows.map(row => ({
                propKey: row.prop_key,
                address: row.property_address || '',
                checkIn: row.check_in,
                checkOut: row.check_out,
            ...
            myGuestReviews = {};
            try { const rv = await apiPost('reviews.php', { action: 'mine' }); myGuestReviews = rv.mine || {}; } catch (e) {}
            ...
            const pendingHtml = pendingMine.map(({ propKey, address, checkIn, checkOut, checkInTime, checkOutTime, adults, children }) => {
                const meta = propertyMeta[propKey] || { name: propKey };
            ...
            const upcomingCards = [], pastCards = [], hubCards = [];
            mine.forEach(({ propKey, booking: b, address, payToken }) => {
                ...
                if (currentStay) {
                    ...
                    const balanceTile = (upcoming && !ps.fullyPaid && payToken)
                        ? `<button class="hub-tile" onclick="openPayView('${payToken}', ${b.dbId}, 'balance')"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="5" width="20" height="14" rx="2.5"/><path d="M2 10h20"/></svg><span>Pay balance</span></button>`
                        : '';
                    hubCards.push(`
        ...
        async function renderInStayTides() {
            ...
            let data;
            try { data = await apiGet('tides.php?start=' + todayDashed() + '&days=1'); }
            catch (e) { els.forEach(el => el.style.display = 'none'); return; }
            if (!data || !data.ok || !Array.isArray(data.extremes)) { els.forEach(el => el.style.display = 'none'); return; }
            const today = todayDashed();
            const lows = [], highs = [];
            data.extremes.forEach(e => {
                ...
                if (/low/i.test(e.type)) lows.push(hm); else if (/high/i.test(e.type)) highs.push(hm);
            });
            if (!lows.length && !highs.length) { els.forEach(el => el.style.display = 'none'); return; }
            const parts = [];
        ...
        function loadLeaflet() {
            ...
            __leafletLoader = new Promise((resolve, reject) => {
                ...
                js.onload = () => resolve();
                js.onerror = () => { __leafletLoader = null; reject(new Error('map load failed')); };
        ...
        function loadSquareSdk(env) {
            ...
            const host = env === 'production' ? 'https://web.squarecdn.com' : 'https://sandbox.web.squarecdn.com';
            __squareSdkLoader = new Promise((resolve, reject) => {
                const js = document.createElement('script');
                js.src = host + '/v1/square.js';
                js.onload = () => resolve();
                js.onerror = () => { __squareSdkLoader = null; reject(new Error('Square could not be reached. Please try again.')); };
                document.head.appendChild(js);
            });
        ...
        function showPayError(text) {
            ['pay-loading', 'pay-body', 'pay-done'].forEach(id => { const e = document.getElementById(id); if (e) e.style.display = 'none'; });
            const err = document.getElementById('pay-error'), msg = document.getElementById('pay-error-msg');
        ...
        async function openPayView(token, bookingId, kind) {
            ...
            squareCard = null;
            ['pay-body', 'pay-done', 'pay-error'].forEach(id => { const e = document.getElementById(id); if (e) e.style.display = 'none'; });
            const ld = document.getElementById('pay-loading'); if (ld) ld.style.display = '';
            const cardEl = document.getElementById('sq-card'); if (cardEl) cardEl.innerHTML = '';
            setPayMsg('');
            nav('view-pay');
            try {
                const cfg = await apiGet('square-config.php');
                if (!cfg.enabled || !cfg.applicationId || !cfg.locationId) throw new Error('Online payment is not available right now.');
                const s = await apiPost('pay.php', { action: 'summary', booking_id: payState.bookingId, token: payState.token, kind: payState.kind });
                ...
                try { await mountWallets(s.amountDue); } catch (e) {}   // Apple/Google Pay (best-effort)
        ...
        async function payWithToken(sourceId) {
            ...
            document.getElementById('pay-done').style.display = '';
            try { toast('Payment received — thank you!'); } catch (e) {}
        }
        ...
        async function mountWallets(amountDue) {
            ...
            const walletPay = async (wallet) => {
                ...
                try {
                    ...
                    if (result.status !== 'OK') throw new Error((result.errors && result.errors[0] && result.errors[0].message) || 'Payment was cancelled.');
                    await payWithToken(result.token);
                } catch (e) { setPayMsg(e.message || 'Payment failed. Please try again.'); }
            };
            try {
                const gp = await squarePayments.googlePay(req);
                const el = document.createElement('div'); el.id = 'sq-gpay'; wrap.appendChild(el);
                await gp.attach('#sq-gpay', { buttonColor: 'black', buttonType: 'long', buttonSizeMode: 'fill' });
                el.addEventListener('click', async (e) => { e.preventDefault(); await walletPay(gp); });
                any = true;
            } catch (e) { const x = document.getElementById('sq-gpay'); if (x) x.remove(); }
            try {
                const ap = await squarePayments.applePay(req);
                const btn = document.createElement('button'); btn.id = 'sq-apay'; btn.type = 'button';
                btn.className = 'sq-apple-btn'; btn.setAttribute('aria-label', 'Pay with Apple Pay');
                wrap.appendChild(btn);
                btn.addEventListener('click', async () => { await walletPay(ap); });
                any = true;
            } catch (e) { const x = document.getElementById('sq-apay'); if (x) x.remove(); }
            const orEl = document.getElementById('sq-or'); if (orEl) orEl.style.display = any ? '' : 'none';
        }
        async function submitPayment() {
            ...
            if (btn) { btn.disabled = true; btn.classList.add('is-busy'); btn.textContent = 'Processing…'; }
            try {
                const result = await squareCard.tokenize();
                if (result.status !== 'OK') {
                    const m = (result.errors && result.errors[0] && result.errors[0].message) || 'Please check your card details and try again.';
                    throw new Error(m);
                }
                await payWithToken(result.token);
            } catch (e) {
                ...
                if (btn) { btn.disabled = false; btn.classList.remove('is-busy'); btn.textContent = orig; }
            }
        }
        ...
        function maybeOpenReviewLink() {
            try {
                ...
                if (currentGuest) { nav('view-guest-bookings'); renderGuestBookings(); }
                else { openGuestArea(); }
                return true;
        ...
        let mapProvider = (() => {
            try { const s = localStorage.getItem('chb-map-provider'); if (s === 'apple' || s === 'google') return s; } catch (e) {}
        ...
        async function getVapidKey() {
            ...
            try { const r = await apiGet('push.php?action=key'); __vapidKey = r.key || ''; }
            catch (e) { __vapidKey = ''; }
            return __vapidKey;
        }
            ...
            if (!window.__chbReloadWired) {
                ...
                navigator.serviceWorker.addEventListener('message', (e) => {
                    if (e.data && e.data.type === 'chb-reload') {
                        try { toast('Updating to the new version…'); } catch (_) {}
                        ...
                        try { oqRefreshCount(); } catch (_) {}
                        try { if (typeof loadExpenses === 'function') loadExpenses().then(() => { if (document.querySelector('#asec-expenses')) renderExpenses(); try { renderMoneyOverview(); } catch (_) {} }); } catch (_) {}
                        try { if (typeof loadAdminMessages === 'function') loadAdminMessages(); } catch (_) {}
                    }
                });
            }
        ...
        function captureAdminState() {
            try {
                ...
                sessionStorage.setItem('chb-admin-state', JSON.stringify({
                    view: av ? av.id : '',
        ...
        async function restoreAdminState() {
            let s = null;
            try { s = JSON.parse(sessionStorage.getItem('chb-admin-state') || 'null'); } catch (_) {}
            try { sessionStorage.removeItem('chb-admin-state'); } catch (_) {}
            ...
            try {
                ...
                if (s.scrollY) setTimeout(() => { try { window.scrollTo({ top: s.scrollY, behavior: 'auto' }); } catch (_) {} }, 350);
            } catch (_) {}
        }
        function reloadForUpdate() { captureAdminState(); try { location.reload(); } catch (_) {} }
        ...
        let __verTimer = null, __verReloading = false;
        ...
        let __gVerTimer = null, __gVerReloading = false, __gLastInput = Date.now();
        const __G_IDLE_MS = 45000;
        ['pointerdown', 'keydown', 'touchstart', 'wheel', 'scroll', 'click'].forEach(function (ev) {
            window.addEventListener(ev, function () { __gLastInput = Date.now(); }, { passive: true });
        });
        function startGuestVersionWatch() {
            ...
            const check = async () => {
                ...
                if (document.querySelector('.modal-overlay.open, .pop-modal.open, #chat-widget.open')) return;  // mid-task
                try {
                    const r = await fetchWithTimeout(API_BASE + 'version.php', { cache: 'no-store' }, 8000);
                    if (!r.ok) return;
                    const d = await r.json();
                    if (d && d.build && window.__BUILD && d.build !== window.__BUILD) {
                        ...
                        try { location.reload(); } catch (_) { __gVerReloading = false; }
                    }
        ...
        async function enableArrivalPush() {
            ...
            } catch (e) { /* push is best-effort; never break the page */ }
        }
        ...
        async function enableGuestPush() {
            try {
                ...
                if (!supported) {
                    if (isAppleDevice() && !standalone) glassAlert('To get notifications on iPhone or iPad, first add this site to your Home Screen (tap Share → Add to Home Screen), then open it from there and try again.');
                    else glassAlert('This device or browser doesn’t support notifications.');
                    return;
                }
                const perm = await Notification.requestPermission();
                if (perm !== 'granted') { glassAlert('Notifications are blocked. You can allow them for this site in your browser settings, then try again.'); dismissGuestPushPrompt(); return; }
            ...
            } catch (e) { glassAlert("Couldn't enable notifications: " + (e.message || e)); }
        }
        // Hide (and remember) the opt-in card so we don't nag on every render.
        function dismissGuestPushPrompt() {
            try { localStorage.setItem('chb-guest-push-prompt', '1'); } catch (e) {}
            const el = document.getElementById('guest-push-prompt'); if (el) el.remove();
        }
        ...
        function guestPushPromptHtml(hasUpcoming) {
            ...
            try { if (localStorage.getItem('chb-guest-push-prompt') === '1') return ''; } catch (e) {}
        ...
        async function enableOwnerPush() {
            try {
                if (!('serviceWorker' in navigator) || !('PushManager' in window) || !('Notification' in window)) { glassAlert('This device or browser doesn’t support notifications.'); return; }
                const perm = await Notification.requestPermission();
                if (perm !== 'granted') { glassAlert('Notifications are blocked. Enable them for this site in your browser settings, then try again.'); renderNotifySettings(); return; }
                const key = await getVapidKey();
                if (!key) { glassAlert('Push isn’t configured on the server yet (VAPID keys in config.php).'); return; }
                const reg = await registerServiceWorker();
                if (!reg) { glassAlert('Could not register the notification worker.'); return; }
                let sub = await reg.pushManager.getSubscription();
        ...
        async function testOwnerPush() {
            try { await apiGet('push.php?action=test_admin'); toast('Test alert sent — check your notifications.'); }
            catch (e) { glassAlert("Couldn't send test: " + (e.message || e)); }
        }
        function renderNotifySettings() {
            ...
            const status = !supported ? 'Not supported on this device or browser.'
                : perm === 'granted' ? 'Notifications are allowed on this browser.'
                : perm === 'denied' ? 'Notifications are blocked — enable them for this site in your browser settings.'
        ...
        function guestReviewButton(propKey) {
            ...
            if (existing.status === 'approved') return `<button class="btn-sm btn-edit" onclick="toggleGuestReview('${propKey}')"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.5l2.6 5.27 5.82.85-4.21 4.1.99 5.78L12 16.9l-5.2 2.6.99-5.78-4.21-4.1 5.82-.85z" fill="currentColor" stroke="none"/></svg> Edit your review</button>`;
            if (existing.status === 'pending') return `<button class="btn-sm btn-edit" onclick="toggleGuestReview('${propKey}')"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.5l2.6 5.27 5.82.85-4.21 4.1.99 5.78L12 16.9l-5.2 2.6.99-5.78-4.21-4.1 5.82-.85z" fill="currentColor" stroke="none"/></svg> Review submitted</button>`;
            return `<button class="btn-sm btn-edit" onclick="toggleGuestReview('${propKey}')"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.5l2.6 5.27 5.82.85-4.21 4.1.99 5.78L12 16.9l-5.2 2.6.99-5.78-4.21-4.1 5.82-.85z" fill="currentColor" stroke="none"/></svg> Edit your review</button>`;
        }
        ...
        async function openWelcomeBook(propKey) {
            ...
            if (bodyEl) bodyEl.innerHTML = `<p style="color:var(--text-muted);font-size:0.9rem;">Loading…</p>`;
            const m = document.getElementById('welcome-modal'); if (m) m.classList.add('open');
            let sections = [];
            try {
        ...
        function closeWelcomeModal() { const m = document.getElementById('welcome-modal'); if (m) m.classList.remove('open'); }
        ...
        function openPhotoUpload(propKey) {
            ...
            const meta = propertyMeta[propKey] || { name: propKey };
            const nameEl = document.getElementById('pu-prop-name'); if (nameEl) nameEl.textContent = meta.name;
            const fileEl = document.getElementById('pu-file'); if (fileEl) fileEl.value = '';
            const capEl = document.getElementById('pu-caption'); if (capEl) capEl.value = '';
            const msg = document.getElementById('pu-msg'); if (msg) { msg.textContent = ''; }
            const m = document.getElementById('photo-upload-modal'); if (m) m.classList.add('open');
        }
        function closePhotoUpload() { const m = document.getElementById('photo-upload-modal'); if (m) m.classList.remove('open'); }
        async function submitGuestPhoto() {
            ...
            if (!file) { show('Please choose a photo.', false); return; }
            if (file.size > 8 * 1024 * 1024) { show('Please choose an image 8 MB or smaller.', false); return; }
            show('Uploading…', true);
            try {
                const fd = new FormData();
                ...
                const r = await fetch(API_BASE + 'photos.php', { method: 'POST', credentials: 'same-origin', body: fd });
            ...
            } catch (e) { show(e.message || 'Could not upload your photo.', false); }
        }
        function guestReviewForm(propKey) {
            const existing = myGuestReviews[propKey];
            const meta = propertyMeta[propKey] || { name: propKey };
            let note = '';
            if (existing && existing.status === 'approved') note = `<div style="font-size:0.82rem;color:#4CAF50;margin-bottom:10px;"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.5l2.6 5.27 5.82.85-4.21 4.1.99 5.78L12 16.9l-5.2 2.6.99-5.78-4.21-4.1 5.82-.85z" fill="currentColor" stroke="none"/></svg> Your review of ${escapeHtml(meta.name)} is live on our home page — thank you!</div>`;
            else if (existing && existing.status === 'pending') note = `<div style="font-size:0.82rem;color:var(--text-muted);margin-bottom:10px;"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.5l2.6 5.27 5.82.85-4.21 4.1.99 5.78L12 16.9l-5.2 2.6.99-5.78-4.21-4.1 5.82-.85z" fill="currentColor" stroke="none"/></svg> Thank you for staying with us!</div>`;
            const stars = existing ? existing.stars : 5;
            const starOpts = [5,4,3,2,1].map(n => `<option value="${n}" ${stars === n ? 'selected' : ''}>${'★'.repeat(n)}${'☆'.repeat(5-n)}</option>`).join('');
            return `
        ...
        function guestReviewBlock(propKey) {
            ...
            if (!existing) {
                ...
                head = `<span style="font-size:0.82rem;color:var(--text-muted);"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.5l2.6 5.27 5.82.85-4.21 4.1.99 5.78L12 16.9l-5.2 2.6.99-5.78-4.21-4.1 5.82-.85z" fill="currentColor" stroke="none"/></svg> Thank you for staying with us!</span>
                        <button class="btn-sm btn-edit" style="margin-left:10px;" onclick="toggleGuestReview('${propKey}')">Edit review</button>`;
            } else {   // declined — let them quietly revise rather than surfacing "declined"
                head = `<button class="btn-sm btn-edit" onclick="toggleGuestReview('${propKey}')"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.5l2.6 5.27 5.82.85-4.21 4.1.99 5.78L12 16.9l-5.2 2.6.99-5.78-4.21-4.1 5.82-.85z" fill="currentColor" stroke="none"/></svg> Edit your review</button>`;
            }
            const stars = existing ? existing.stars : 5;
            const starOpts = [5,4,3,2,1].map(n => `<option value="${n}" ${stars === n ? 'selected' : ''}>${'★'.repeat(n)}${'☆'.repeat(5-n)}</option>`).join('');
        ...
        async function submitGuestReview(propKey) {
            ...
            if (text.length < 10) { glassAlert('Please write at least a sentence or two.'); return; }
            try {
            ...
            } catch (e) { glassAlert("Couldn't submit your review: " + e.message); }
        }
        ...
        function cancellationClauseParagraphs(propKey) {
            const pol = CANCELLATION_POLICIES[cancelPolicyOf(propKey)] || CANCELLATION_POLICIES[DEFAULT_CANCEL_POLICY];
        ...
        function effectiveTermsSections(propKey) {
            return termsSections.map(s => s.h.startsWith('7.')
                ? { h: s.h, p: cancellationClauseParagraphs(propKey) }
                : s);
        }
        function renderTerms(propKey) {
            ...
            if (!body) return;
            body.innerHTML = effectiveTermsSections(propKey).map(s => {
                const isDefs = s.h.startsWith('1.');
                const paras = s.p.map(par => {
        ...
        async function sendArrivalInfo(bookingId) {
            ...
            if (!b) { glassAlert("Couldn't find that booking to email."); return; }
            if (!b.email) { glassAlert("This booking has no guest email on file."); return; }
            if (!await glassConfirm(`Send the arrival info email to ${b.email}?\n\nTip: the arrival details are set per cottage in Settings & Fees.`)) return;
            try {
            ...
            } catch (e) { glassAlert("Couldn't send: " + e.message); }
        }
        ...
        function downloadInvoice(bookingId) {
            ...
            if (!b) { glassAlert('Booking not found.'); return; }
            const meta = propertyMeta[propKey];
            const r = propertyRates[propKey] || defaultRates[propKey];
            ...
            // Invoice meta
            doc.setFontSize(10);
            rowLR('Invoice reference', bookingRef(b.id), y); y += 18;
            rowLR('Issued', todayDashed(), y); y += 18;
            rowLR('Guest', b.name || '', y); y += 18;
            if (b.email) { rowLR('Email', b.email, y); y += 18; }
            y += 10; line(y); y += 28;

            // Stay details
            doc.setFont('helvetica', 'bold'); doc.setFontSize(12);
            doc.text('Your Stay', left, y); y += 20;
            doc.setFontSize(10);
            rowLR('Property', meta.name, y); y += 18;
            // Address can wrap
            doc.setFont('helvetica', 'normal');
            doc.text('Address', left, y);
            const addrLines = doc.splitTextToSize(address || 'Address provided on confirmation.', 300);
            doc.text(addrLines, right, y, { align: 'right' }); y += (addrLines.length * 14) + 6;
            rowLR('Check in', `${b.checkIn}  ·  ${b.checkInTime || '15:00'}`, y); y += 18;
            rowLR('Check out', `${b.checkOut}  ·  ${b.checkOutTime || '10:00'}`, y); y += 18;
            rowLR('Nights', String(p.nights), y); y += 18;
            rowLR('Guests', b.guests || `${b.adults || 0} adults`, y); y += 18;
            y += 10; line(y); y += 28;

            // Charges
            doc.setFont('helvetica', 'bold'); doc.setFontSize(12);
            doc.text('Charges', left, y); y += 20;
            doc.setFontSize(10);
            rowLR(`${gbp(p.perNight)} x ${p.nights} night${p.nights === 1 ? '' : 's'}`, gbp(p.nightly), y); y += 18;
            rowLR(`Transaction fee (${p.transactionPct}%)`, gbp(p.txFee), y); y += 18;
            rowLR('Refundable damages deposit', gbp(p.damagesDeposit), y); y += 18;
            y += 4; line(y); y += 20;
            rowLR('Total', gbp(p.total), y, true); y += 22;

            // Payments
            doc.setFont('helvetica', 'bold'); doc.setFontSize(12);
            doc.text('Payments', left, y); y += 20;
            doc.setFontSize(10);
            if (ps.deposit > 0) {
                const how = b.paymentMethod ? ` via ${b.paymentMethod}` : '';
                const when = b.paymentDate ? ` on ${b.paymentDate}` : '';
                rowLR(`Amount paid${how}${when}`, '- ' + gbp(ps.deposit), y); y += 18;
                rowLR(ps.fullyPaid ? 'Paid in full' : 'Balance due', gbp(ps.fullyPaid ? ps.total : ps.balance), y, true); y += 18;
            } else {
                rowLR('Amount paid', gbp(0), y); y += 18;
                rowLR('Balance due', gbp(ps.balance), y, true); y += 18;
            }

            // Footer
            y += 30; line(y); y += 20;
            doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(120);
            doc.text('Thank you for booking with Cottage Holidays Blakeney. We look forward to welcoming you.', left, y);
        ...
        function autofillGuestEnquiry() {
            ...
            if (!currentGuest) {
                ...
                if (e) { e.readOnly = false; e.classList.remove('input-locked'); e.title = ''; }
                return;
            }
        ...
        async function openAdminLogin(title, sub, onSuccess) {
            ...
            const hasPasskey = !!(window.PublicKeyCredential && navigator.credentials && navigator.credentials.get);
            ...
            if (hasPasskey) {
                ...
                } catch (e) { /* unavailable / cancelled / failed — fall through to password */ }
        ...
        async function submitAdminLogin() {
            ...
            if (!username || !password) { adminLoginErr('Please enter your username and password.'); return; }
            try {
                await apiPost('auth.php', { action: 'admin_login', username, password });
                isAuthenticated = true; setAuthUI();
                currentGuest = null; setGuestUI();   // one role at a time: drop any guest session
                const cb = adminLoginOnSuccess;
                closeAdminLogin();
                if (cb) await cb();
            } catch (e) { adminLoginErr('Access denied: ' + e.message); }
        }
        async function submitAdminPasskey() {
            ...
            } catch (e) { adminLoginErr('Passkey sign-in failed: ' + (e.message || e)); }
        }
        ...
        async function adminPasskeyFirst(skipConfirm) {
            ...
            if (!skipConfirm && !await glassConfirm("Sign in with a passkey?\n\nOK = use Face ID / Touch ID / device PIN\nCancel = use username & password")) return false;
            try {
                ...
                if (!skipConfirm) glassAlert("Passkey sign-in failed: " + (e.message || e) + "\n\nYou can use your username and password instead.");
                else throw e;
                return false;
            }
        }
        ...
        async function loadContent() {
            ...
            } catch (e) { return; }   // keep last-good content on a transient failure (e.g. background poll)
        ...
        async function liveUpdateTick() {
            ...
            try {
                ...
                try { loadPropContentOverrides(); } catch (e) {}
                try { renderReviews(); } catch (e) {}
                try { updatePropPriceHeading(); } catch (e) {}
                try { if (document.getElementById('enq-price-box')) updateEnquiryPrice(); } catch (e) {}
                // Keep the cottage calendar / date picker availability fresh.
                try { if (activeFrontProperty) await loadAvailability(activeFrontProperty); } catch (e) {}
                try { if (activeFrontProperty) renderPropReviews(activeFrontProperty); } catch (e) {}
            } finally {
        ...
        function startLiveUpdates() {
            ...
            document.addEventListener('visibilitychange', () => { if (!document.hidden) liveUpdateTick(); });
        }
        ...
        window.addEventListener('DOMContentLoaded', async () => {
          try {
            ...
            try { const em = document.getElementById('enquire-modal'); if (em && em.parentElement !== document.body) document.body.appendChild(em); } catch (e) {}
            try { const bb = document.getElementById('prop-book-bar'); if (bb && bb.parentElement !== document.body) document.body.appendChild(bb); } catch (e) {}
            ...
            try { renderCardPrices(); } catch (e) { console.error(e); }
            try { loadPropContentOverrides(); } catch (e) { console.error(e); }
            try { renderReviews(); } catch (e) { console.error(e); }
            try { injectSeoSchema(); } catch (e) { console.error(e); }   // live review stars / FAQs / phone into structured data
            try { updatePropPriceHeading(); } catch (e) { console.error(e); }
            try { if (document.getElementById('enq-price-box')) updateEnquiryPrice(); } catch (e) { console.error(e); }
            ...
            try {
                if (currentGuest && 'Notification' in window && Notification.permission === 'granted') enableArrivalPush();
            ...
            try { const av = document.querySelector('.page-view.active'); trackView(av ? av.id : 'view-main', activeFrontProperty); } catch (e) {}
            try { startLiveUpdates(); } catch (e) { console.error(e); }
            try { startVersionWatch(); } catch (e) { console.error(e); }
            try { startGuestVersionWatch(); } catch (e) { console.error(e); }
            // After an auto-update reload, return the owner to the screen they were on.
            try { restoreAdminState(); } catch (e) { console.error(e); }
            try { maybeConsumeMagicLink(); } catch (e) { console.error(e); } // ?mlogin=… → passwordless sign in
        ...
        function hideLoadingOverlay() {
            ...
            setTimeout(() => { if (o.parentNode) o.parentNode.removeChild(o); }, 600);
        }
        ...
        function checkBookingRules(propKey, checkIn, checkOut) {
            ...
            if (allowed.length > 0) {
                ...
                if (!allowed.includes(arrivalDow)) {
                    const names = allowed.slice().sort((a,b)=>a-b).map(i => DAY_NAMES[i]).join(', ');
        ...
        const termsSections = [
            ...
            { h: '3. Looking after the property', p: [
            ...
            { h: '6. Our responsibility to you', p: [
            ...
            { h: '9. Events outside anyone’s control', p: [
            ...
            { h: '10. Your personal information', p: [
                'We use the information you give us in line with our Privacy Policy.'
            ]},
            { h: '11. Changes to your booking or these terms', p: [
        ...
        const propertyContent = {
            ...
            'jollyboat': {
                ...
                amenities: ['Snug Double Bedroom', 'Wood-Burning Stove', 'Quayside Location', 'Welcome Hamper'],
                images: [
                    'jollyboat-1.jpg',
                    'jollyboat-2.jpg',
                    'jollyboat-3.jpg'
        ...
        function renderSafety(propKey) {
            ...
            if (!wrap) return;
            wrap.innerHTML = activePropSafety.map(s => `
        ...
        const HOST_DEFAULTS = {
            'host-name': 'Sophia', 'host-badge': 'Owner', 'host-reviews': '56',
            'host-rating': '4.95 ★', 'host-years': '10 years',
            'host-school': 'Where I studied: North Norfolk',
            'host-work': 'My work: holiday accommodation',
            'host-bio': "Hi — you can take the girl out of Norfolk, but you can't take Norfolk out of the girl. I love helping guests discover this beautiful stretch of coast.",
            'host-photo': ''
        };
        const hostVal = (k) => { const v = siteContent[k]; return (v === undefined || v === null || v === '') ? HOST_DEFAULTS[k] : v; };
        // Fill the cottage-page host card from saved content (no longer inline-editable).
        function renderHost() {
            const set = (id) => { const e = document.getElementById(id); if (e) e.textContent = hostVal(id); };
        ...
        function saveHostText(key, value) {
            ...
            if (msg) { msg.textContent = 'Saved.'; clearTimeout(msg.__t); msg.__t = setTimeout(() => { msg.textContent = ''; }, 1500); }
        }
        function uploadHostPhoto() {
            pickAndUpload('host-photo', async (url) => {
                siteContent['host-photo'] = url;
                await saveContent('host-photo', url);
                const a = document.getElementById('host-edit-photo'); if (a) a.style.backgroundImage = `url('${url}')`;
                renderHost();
            });
        }
        // Host profile now lives inside Settings; this shim keeps any old callers working.
        function openHostEditor() { openSettings('host'); }

        // ---- Local guide (dark skies + car-free) + book-direct savings badge ----
        const DEFAULT_DARKSKIES = "North Norfolk has some of England's darkest skies. On a clear night, step outside and look up — you can often see the Milky Way. Give your eyes 15 minutes to adjust and bring a blanket.";
        const DEFAULT_CARFREE = "You can reach us car-free: train to Sheringham, then the Coasthopper bus along the coast to Blakeney. The Norfolk Coast Path runs through the village, so it's an easy base for walking to Cley, Morston and Wells.";
        const DEFAULT_ACCESS = "Please ask us before you book if you have specific access needs — we're happy to talk through the layout. Tell us about parking distance, steps or stairs, doorway widths, ground-floor sleeping and bathroom facilities so we can confirm the cottage is right for you.";
        function saveLocalContent(key, value) { saveContent(key, value); try { renderLocalGuide(activeFrontProperty); } catch (e) {} }
        ...
        async function renderTides() {
            ...
            try {
                ...
                if (!__tideData || !__tideData.ok || !Array.isArray(__tideData.extremes)) { hide(); return; }
                const now = Date.now();
                ...
                if (!next.length) { hide(); return; }
                body.innerHTML = next.map(e => {
        ...
        function renderLocalGuide(propKey) {
            ...
            const dkEl = document.getElementById('prop-darkskies'); if (dkEl) dkEl.textContent = dk;
            const cfEl = document.getElementById('prop-carfree'); if (cfEl) cfEl.textContent = cf;
            const acEl = document.getElementById('prop-access'); if (acEl) acEl.textContent = ac;
        ...
        async function renderGuestPhotos(propKey) {
            ...
            const hide = () => { section.style.display = 'none'; if (divider) divider.style.display = 'none'; };
            try {
        ...
        function openPhotoLightbox(data) {
            ...
            const img = document.getElementById('pl-img'); const c = document.getElementById('pl-cap');
            if (img) img.src = url; if (c) c.textContent = cap || '';
            if (box) {
                box.classList.add('open');
                document.body.style.overflow = 'hidden';   // lock scroll behind the lightbox
                const close = box.querySelector('.pl-close'); if (close) close.focus();
            }
        }
        function closePhotoLightbox() {
            const box = document.getElementById('photo-lightbox'); if (box) box.classList.remove('open');
            const img = document.getElementById('pl-img'); if (img) img.src = '';
            document.body.style.overflow = '';
        }
        ...
        const DEFAULT_TRIP_ACTIVITIES = [
            ...
            { name: 'Cosy day in', blurb: 'Wood-burner, a good book and the rain on the window.', tags: ['rainy'], tide: 'any' },
            { name: 'Muckleburgh Collection', blurb: 'Military vehicle museum near Weybourne — a great rainy-day option.', tags: ['rainy', 'kids'], tide: 'any' },
        ];
        ...
        function openTripModal() {
            ...
            if (wrap) wrap.innerHTML = TRIP_INTERESTS.map(([id, label]) =>
                `<button type="button" class="trip-chip" data-int="${id}" onclick="this.classList.toggle('on')">${label}</button>`).join('');
            const sel = document.getElementById('trip-days');
            if (sel) sel.innerHTML = [2, 3, 4, 5, 6, 7].map(n => `<option value="${n}" ${n === 3 ? 'selected' : ''}>${n}</option>`).join('');
            const res = document.getElementById('trip-result'); if (res) res.innerHTML = '';
            const m = document.getElementById('trip-modal'); if (m) m.classList.add('open');
        }
        function closeTripModal() { const m = document.getElementById('trip-modal'); if (m) m.classList.remove('open'); }
        function runTripPlan() {
            const interests = Array.from(document.querySelectorAll('#trip-interests .trip-chip.on')).map(b => b.getAttribute('data-int'));
            const days = Math.max(1, Math.min(7, parseInt((document.getElementById('trip-days') || {}).value, 10) || 3));
            const plan = generateTripPlan({ interests, days });
            const res = document.getElementById('trip-result');
            if (!res) return;
            if (!plan.length) { res.innerHTML = `<p style="color:var(--text-muted);font-size:0.9rem;">Pick an interest or two and tap “Plan my days”.</p>`; return; }
            res.innerHTML = plan.map((items, di) => `
        ...
        function curatedTripPlan({ interests, days }) {
            ...
            let matched = (interests && interests.length) ? pool.filter(a => (a.tags || []).some(t => interests.includes(t))) : pool.slice();
            ...
            const tideNote = (a) => {
                ...
                if (__tideData && __tideData.ok && Array.isArray(__tideData.extremes)) {
                    const ex = __tideData.extremes.map(e => ({ t: new Date(e.time), type: e.type }))
                        .filter(e => new RegExp(want, 'i').test(e.type) && e.t.getTime() >= Date.now())
                        .sort((a, b) => a.t - b.t);
                    if (ex.length) { const d = ex[0].t; return ` · best near ${want} tide (~${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')})`; }
                }
                return a.tide === 'high' ? ' · best around high tide' : ' · best around low tide';
            };
            const perDay = 2, plan = []; let i = 0;
        ...
        function renderHouseRules(propKey) {
            ...
            const item = (text) => `<div class="things-item">${IC_CHECK} <span>${escapeHtml(text)}</span></div>`;
            ...
            const custom = Array.isArray(siteContent['houserules-' + propKey]) ? siteContent['houserules-' + propKey] : DEFAULT_HOUSE_RULES;
            wrap.innerHTML = [
                item(`Check-in after ${ci}`),
                item(`Checkout before ${co}`),
                maxG ? item(`${maxG} guest${maxG === 1 ? '' : 's'} maximum`) : ''
            ].concat(custom.map(item)).join('');
        }
        ...
        async function renderLocationMap(propKey) {
            ...
            if (!geo || typeof geo.lat === 'undefined' || typeof geo.lng === 'undefined') {
                el.classList.add('is-empty'); return;   // no coordinates set -> show the address text only
            }
            el.classList.remove('is-empty');
            try { await loadLeaflet(); } catch (e) { el.classList.add('is-empty'); return; }
            if (__propMap) { try { __propMap.remove(); } catch (e) {} __propMap = null; }
            el.innerHTML = '';
            const map = L.map(el, { zoomControl: true, attributionControl: true, scrollWheelZoom: false });
            map.attributionControl.setPrefix('');
            L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png', { maxZoom: 20, subdomains: 'abcd', attribution: '© OpenStreetMap, © CARTO' }).addTo(map);
            map.setView([geo.lat, geo.lng], 15);
            const pin = L.divIcon({ className: 'prop-pin', iconSize: [42, 42], iconAnchor: [21, 21],
        ...
        const defaultRates = {
            '21a':       { coupleRate: 130, extraAdultRate: 45, childRate: 30, damagesDeposit: 75, transactionPct: 3, checkInTime: '15:00', checkOutTime: '10:00', minNights: 2, maxNights: 0, arrivalDays: [], address: '21A Westgate Street, Blakeney, Norfolk NR25 7NQ' },
            'jollyboat': { coupleRate: 110, extraAdultRate: 40, childRate: 25, damagesDeposit: 75, transactionPct: 3, checkInTime: '15:00', checkOutTime: '10:00', minNights: 2, maxNights: 0, arrivalDays: [], address: 'Jollyboat, Quay Road, Blakeney, Norfolk NR25 7ND' },
            'pimpernel': { coupleRate: 120, extraAdultRate: 42, childRate: 28, damagesDeposit: 75, transactionPct: 3, checkInTime: '15:00', checkOutTime: '10:00', minNights: 2, maxNights: 0, arrivalDays: [], address: 'Pimpernel, High Street, Cley-next-the-Sea, Norfolk NR25 7RF' }
        };
        ...
        function priceBreakdown(propKey, adults, children, checkIn, checkOut, depositOverride) {
            ...
            const depBase = (depositOverride != null && depositOverride !== '') ? parseFloat(depositOverride) : r.damagesDeposit;
        ...
        function snapshotPrice(propKey, booking) {
            const p = priceBreakdown(propKey, booking.adults, booking.children, booking.checkIn, booking.checkOut, booking.damagesDeposit);
            booking.agreedPrice = {
                nights: p.nights,
                perNight: p.perNight,
                nightly: p.nightly,
        ...
        async function reconcileDeposit(propKey, booking, status) {
            const p = booking.agreedPrice || priceBreakdown(propKey, booking.adults || 0, booking.children || 0, booking.checkIn, booking.checkOut);
            const total = p.total;
            ...
            const entered = await glassPrompt(`Deposit paid (£). Must be between £0.01 and ${gbp(total - 0.01)}:`, '');
            if (entered === null) return null;
            let val = Math.max(0, parseFloat(entered) || 0);
            if (val <= 0 || val >= total) {
                glassAlert("A deposit must be more than £0 and less than the full total. Use 'Paid in Full' or 'Unpaid' instead.");
                return null;
            }
        ...
        function persistDB() { /* server is the source of truth */ }
        function persistEnquiries() { /* server is the source of truth */ }
        ...
        function suppressBlocksUnderLocalBookings() {
            Object.keys(dbBlocks).forEach(k => {
                ...
                dbBlocks[k] = (dbBlocks[k] || []).filter(bl =>
                    !locals.some(bk => rangesOverlap(bl.checkIn, bl.checkOut, bk.checkIn, bk.checkOut))
                );
            });
        }
        ...
        function sourceLabel(src) {
            ...
            const map = { airbnb: 'Airbnb', vrbo: 'Vrbo', 'booking': 'Booking.com', bookingcom: 'Booking.com', google: 'Google' };
        ...
        async function updatePaymentStatus(bookingId, newStatus) {
            ...
            if (newStatus === 'deposit' || newStatus === 'paid') {
                const d = await glassPrompt('Payment date (YYYY-MM-DD):', booking.paymentDate || todayDashed());
                if (d === null) { afterPaymentChange(bookingId); return; }
                if (!/^\d{4}-\d{2}-\d{2}$/.test(d.trim())) { glassAlert('A valid payment date is required.'); afterPaymentChange(bookingId); return; }
            ...
            } catch (e) { glassAlert("Couldn't update payment: " + e.message); afterPaymentChange(bookingId); }
        }
        ...
        async function updateDeposit(bookingId, value) {
            ...
            try {
                ...
                const fresh = findBookingById(bookingId);
                if (fresh) afterPaymentChange(bookingId);
            } catch (e) { glassAlert("Couldn't update deposit: " + e.message); afterPaymentChange(bookingId); }
        }
        ...
        async function updatePaymentField(bookingId, field, value) {
            ...
            if (field === 'paymentDate' && !value && Number(booking.depositPaid) > 0) {
                glassAlert('A payment date is required while a payment is recorded. Set the deposit to £0 first if you need to remove it.');
                afterPaymentChange(bookingId);
                return;
            }
            ...
            try {
                ...
                const fresh = findBookingById(bookingId);
                if (fresh) afterPaymentChange(bookingId);
            } catch (e) { glassAlert("Couldn't update: " + e.message); afterPaymentChange(bookingId); }
        }

        // ---- Square online payments (admin) ----
        let squareAdminEnabled = false;
        async function loadSquareAdminConfig() {
            try { const c = await apiGet('square-config.php'); squareAdminEnabled = !!c.enabled; }
            catch (e) { squareAdminEnabled = false; }
            try { renderSquareSettings(); } catch (e) {}
        }
        function renderSquareSettings() {
            const st = document.getElementById('sq-settings-status');
            if (st) st.innerHTML = squareAdminEnabled
            ...
            if (inp) { const v = parseFloat(siteContent['square-deposit-pct']); inp.value = (v > 0 && v <= 100) ? v : 25; }
        }
        async function saveDepositPct() {
            const v = Math.round(parseFloat((document.getElementById('sq-deposit-pct') || {}).value) || 0);
            if (!(v >= 1 && v <= 100)) { glassAlert('Enter a deposit percentage between 1 and 100.'); return; }
            try { await saveContent('square-deposit-pct', v); siteContent['square-deposit-pct'] = v; toast('Deposit policy saved.'); }
            catch (e) { glassAlert("Couldn't save: " + e.message); }
        }
        ...
        async function requestPayment(bookingId, kind) {
            ...
            if (!booking) return;
            try {
                const res = await apiPost('bookings.php', { action: 'request_payment', id: booking.dbId, kind });
                toast(`${kind === 'balance' ? 'Balance' : 'Deposit'} request sent — ${gbp(res.amount)}.`);
            } catch (e) { glassAlert("Couldn't send the payment request: " + e.message); }
        }
        ...
        async function loadBookingPayments(bookingId) {
            ...
            } catch (e) { el.textContent = 'Could not load payments.'; }
        }
        async function refundPayment(bookingId, squareId, maxAmount) {
            ...
            const loc = findBookingLocation(bookingId);
            const propKey = loc ? loc.propKey : '21a';
            const entered = await glassPrompt(`Refund amount (£). Up to ${gbp(maxAmount)}:`, String(maxAmount));
            if (entered === null) return;
            const amount = Math.round((parseFloat(entered) || 0) * 100) / 100;
            if (!(amount > 0 && amount <= maxAmount + 0.001)) { glassAlert(`Enter an amount between £0 and ${gbp(maxAmount)}.`); return; }
            ...
            try {
                ...
                const fresh = findBookingById(bookingId);
                if (fresh) afterPaymentChange(bookingId);
            } catch (e) { glassAlert("Refund failed: " + e.message); }
        }
        ...
        function renderTodayPanel() {
            ...
            if (!el) return;
            const today = todayDashed();
            const in7 = formatDashed(new Date(dpParse(today).getFullYear(), dpParse(today).getMonth(), dpParse(today).getDate() + 7));
            const arrivals = [], departures = [], dueSoon = [], toReturn = [];
            Object.keys(dbBookings).forEach(propKey => {
                (dbBookings[propKey] || []).forEach(b => {
                    ...
                    if (b.checkIn === today) arrivals.push(`${tag} ${escapeHtml(name)} · ${b.checkInTime || '15:00'}`);
                    if (b.checkOut === today) departures.push(`${tag} ${escapeHtml(name)} · ${b.checkOutTime || '10:00'}`);
                    const ps = paymentSummary(propKey, b);
                    if (!ps.fullyPaid && b.checkIn >= today && b.checkIn <= in7) dueSoon.push(`${tag} ${escapeHtml(name)} · ${gbp(ps.balance)} (${b.checkIn})`);
                    const dh = damageHeld(propKey, b);
                    if (dh.held > 0 && (b.checkOut || '') < today) toReturn.push(`${tag} ${escapeHtml(name)} · ${gbp(dh.held)}`);
                });
            });
            const card = (label, items, accent, target) => {
                const click = target ? ` clickable" role="button" tabindex="0" onclick="dashGo('${target}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();dashGo('${target}')}"` : '"';
                return `
        ...
        async function openBlockDates() {
            const names = Object.keys(propertyMeta).map(k => `${k} = ${propertyMeta[k].name}`).join(', ');
            const prop = await glassPrompt(`Which cottage to block? Enter its key (${names}):`, '21a');
            if (prop === null) return;
            const key = (prop || '').trim();
            if (!propertyMeta[key]) { glassAlert('Unknown cottage key. Use one of: ' + Object.keys(propertyMeta).join(', ')); return; }
            ...
            if (to === null) return;
            if (!/^\d{4}-\d{2}-\d{2}$/.test(from.trim()) || !/^\d{4}-\d{2}-\d{2}$/.test(to.trim())) { glassAlert('Please enter valid dates (YYYY-MM-DD).'); return; }
            try {
                await apiPost('ical-import.php', { action: 'add_block', prop: key, check_in: from.trim(), check_out: to.trim() });
                toast('Dates blocked.');
                await initBackOffice();
            } catch (e) { glassAlert("Couldn't block those dates: " + e.message); }
        }
        ...
        function renderCalUpdated() {
            ...
            if (!el) return;
            if (icalSyncing) { el.textContent = 'External calendars: updating…'; return; }
            let last = 0;
            try { last = parseInt(localStorage.getItem(ICAL_LAST_SYNC_KEY) || '0', 10); } catch (e) {}
            el.textContent = last
        ...
        function saveDismissedChangeovers(list) {
            try { localStorage.setItem(CHANGEOVER_DISMISSED_KEY, JSON.stringify(list)); } catch (e) {}
        }
            ...
            Object.keys(dbBookings).forEach(propKey => {
                ...
                list.forEach(leaving => {
                    list.forEach(arriving => {
                        ...
                        if (leaving.checkOut && leaving.checkOut === arriving.checkIn && leaving.checkOut >= todayStr) {
                            out.push({
                                key: `${propKey}|${leaving.checkOut}|${leaving.id}|${arriving.id}`,
                                propKey, date: leaving.checkOut, leaving, arriving
                            });
                        }
                    });
                });
        ...
        function dismissChangeover(key, el) {
            const dismissed = loadDismissedChangeovers();
            if (!dismissed.includes(key)) { dismissed.push(key); saveDismissedChangeovers(dismissed); }
            if (el) {
        ...
        function fmtMsgTime(at) {
            try { const d = new Date(String(at).replace(' ', 'T')); if (isNaN(d)) return ''; return d.toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }); }
            catch (e) { return ''; }
        }
        ...
        function chatNewToken() {
            ...
            try { localStorage.setItem('chb-chat-token', t); } catch (e) {}
            return t;
        }
        function toggleChat() {
            const w = document.getElementById('chat-widget');
            if (!w) return;
            if (w.classList.contains('open')) { closeChat(); return; }
            w.classList.add('open');
            document.getElementById('chat-fab').classList.add('hidden');
            try { if (window.setGuestDockOverlay) window.setGuestDockOverlay('messages'); } catch (e) {}
            loadChat();
            chatStartPolling();   // keep the thread live while it's open
        }
        function closeChat() {
            const w = document.getElementById('chat-widget'); if (w) w.classList.remove('open');
            const f = document.getElementById('chat-fab'); if (f) f.classList.remove('hidden');
            try { if (window.setGuestDockOverlay) window.setGuestDockOverlay(null); } catch (e) {}
            chatStopPolling();
        }
        ...
        function chatMsgSig(msgs) {
            ...
            return msgs.length + ':' + (last.at || '') + ':' + (last.role || '') + ':' + ((last.body || '').length);
        }
        ...
        function chatStartPolling() {
            chatStopPolling();
            __chatPollTimer = setInterval(chatPoll, 15000);
        }
        ...
        async function chatPoll() {
            const w = document.getElementById('chat-widget');
            if (!w || !w.classList.contains('open')) { chatStopPolling(); return; }
        ...
        function chatQuick(text) {
            ...
            if (!input) return;
            if (text === '__issue') { input.value = '🛠 Issue: '; input.focus(); return; }
            input.value = text;
            sendChat();
        }
        ...
        const CHAT_FAQ = {
            checkin: { q: 'What time is check-in and check-out?', key: 'chat-ans-checkin',
        ...
        function chatAppendMe(text) {
            const t = chatThreadEl(); if (!t) return;
            const d = document.createElement('div'); d.className = 'chat-msg me'; d.textContent = text;
            t.appendChild(d); chatScroll();
        }
        function chatBot(html) {
            const t = chatThreadEl(); if (!t) return null;
            const d = document.createElement('div'); d.className = 'chat-bot';
            d.innerHTML = html + '<div class="cb-meta">Quick answer — type below to reach a person.</div>';
            t.appendChild(d); chatScroll(); return d;
        }
        function chatFaq(which) {
            const f = CHAT_FAQ[which]; if (!f) return;
            chatClearEmpty();
            chatAppendMe(f.q);
        ...
        function chatAvailStart() {
            ...
            const opts = Object.keys(propertyMeta).map(k =>
                `<option value="${k}"${k === activeFrontProperty ? ' selected' : ''}>${escapeHtml(propertyMeta[k].name)}</option>`).join('');
            const today = todayDashed();
            chatBot(
        ...
        async function chatAvailRun(uid) {
            ...
            if (!prop || !ci || !co) { chatBot('Please choose a cottage and both dates.'); return; }
            if (co <= ci) { chatBot('Your check-out date needs to be after your check-in date.'); return; }
            try { await loadAvailability(prop); } catch (e) {}
            ...
            if (clash) {
                chatBot(`Sorry — ${escapeHtml(name)} isn't free for ${span}; those dates overlap an existing booking. Try different dates, or I can let you know if they free up.` +
                    `<div class="chat-bot-actions"><button type="button" class="btn-glass" onclick="chatAvailNotify('${prop}','${ci}','${co}')">Notify me</button></div>`);
            } else {
                chatBot(`Good news — ${escapeHtml(name)} looks free for ${span} (${nm} night${nm === 1 ? '' : 's'}). Shall I start your enquiry? No payment is taken now.` +
        ...
        function chatAvailEnquire(prop, ci, co) {
            closeChat();
            try { openProperty(prop); } catch (e) {}
            const set = (id, v) => { const el = document.getElementById(id); if (el) el.value = v; };
            set('enq-checkin', ci); set('enq-checkout', co);
            try { openEnquireModal(); } catch (e) {}
        }
        function chatAvailNotify(prop, ci, co) {
            closeChat();
            try { openWaitlistModal({ prop, checkIn: ci, checkOut: co }); } catch (e) {}
        }
        ...
        function adminCanned(text) {
            ...
            const s = document.getElementById('msg-canned'); if (s) s.value = '';
        }
        async function sendChat() {
            ...
            if (loggedIn) {
                ...
                const nameEl = document.getElementById('chat-name'), emailEl = document.getElementById('chat-email');
                const email = (emailEl && emailEl.value || '').trim();
                if (!token) {   // first message from an anonymous visitor
                    if (email && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) { glassAlert('Please enter a valid email so we can reply.'); return; }
                    token = chatNewToken();
                }
                payload = { action: 'send', token, body, name: (nameEl && nameEl.value || '').trim(), email, ref: document.referrer || '' };
            }
            try {
                const r = await apiPost('messages.php', payload);
                if (r && r.token) { try { localStorage.setItem('chb-chat-token', r.token); } catch (e) {} }
                if (input) input.value = '';
                const intro = document.getElementById('chat-intro'); if (intro) intro.style.display = 'none';
                await loadChat();
            } catch (e) { glassAlert("Couldn't send: " + e.message); }
        }
        ...
        function openWaitlistModal(prefill) {
            ...
            if (sel) {
                sel.innerHTML = Object.keys(propertyMeta).map(k => `<option value="${k}">${escapeHtml(propertyMeta[k].name)}</option>`).join('');
                if (prefill.prop && propertyMeta[prefill.prop]) sel.value = prefill.prop;
            }
            const set = (id, v) => { const e = document.getElementById(id); if (e) e.value = v || ''; };
            set('wl-checkin', prefill.checkIn); set('wl-checkout', prefill.checkOut);
            set('wl-name', currentGuest ? currentGuest.name : '');
            set('wl-email', currentGuest ? currentGuest.email : '');
            const msg = document.getElementById('wl-msg'); if (msg) { msg.textContent = ''; msg.classList.remove('show'); }
            document.getElementById('waitlist-modal').classList.add('open');
        }
        function closeWaitlistModal() { const m = document.getElementById('waitlist-modal'); if (m) m.classList.remove('open'); }
        async function submitWaitlist() {
            const v = id => (document.getElementById(id) || {}).value;
            const msg = document.getElementById('wl-msg');
            const show = t => { if (msg) { msg.textContent = t; msg.classList.add('show'); } };
            const email = (v('wl-email') || '').trim();
            if (!email || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) { show('Please enter a valid email address.'); return; }
            try {
                await apiPost('waitlist.php', { action: 'join', prop: v('wl-prop'), name: v('wl-name'), email, check_in: v('wl-checkin'), check_out: v('wl-checkout') });
                closeWaitlistModal();
                toast("You're on the waitlist — we'll email you if those dates free up.");
            } catch (e) { show(e.message || 'Could not join the waitlist.'); }
        }
        ...
        async function submitNewsletter(ev) {
            ...
            try {
                await apiPost('newsletter.php', { action: 'subscribe', email, name: (currentGuest && currentGuest.name) || '', source: 'footer' });
                if (el) el.value = '';
                show("You're in — thank you! We'll only email occasionally.", true);
            } catch (e) { show(e.message || 'Could not sign you up just now.', false); }
        }
        ...
        async function maybeHandleUnsubscribe() {
            ...
            if (!token) return;
            try { await apiPost('newsletter.php', { action: 'unsubscribe', token }); } catch (e) {}
            try { toast("You've been unsubscribed — sorry to see you go."); } catch (e) {}
        ...
        async function loadAdminMessages() {
            ...
            let threads = [];
            try { const r = await apiPost('messages.php', { action: 'threads', archived: __msgShowArchived ? 1 : 0 }); threads = r.threads || []; }
            catch (e) { if (list) list.innerHTML = `<p style="font-size:0.82rem;color:var(--text-muted);">Couldn't load messages.</p>`; return; }
            // The settings badge only ever reflects unread in the active inbox.
            if (!__msgShowArchived && badge) { const unread = threads.reduce((s, t) => s + (t.unread || 0), 0); badge.textContent = unread; badge.classList.toggle('zero', unread === 0); }
            if (!list) return;
            ...
            const rows = threads.length ? threads.map(t => `
                ...
                </button>`).join('') : `<p style="font-size:0.82rem;color:var(--text-muted);">${__msgShowArchived ? 'No archived conversations.' : 'No messages yet.'}</p>`;
        ...
        function renderChatAnswersEditor() {
            const host = document.getElementById('chat-answers-editor'); if (!host) return;
            host.innerHTML = '<h3 style="font-family:var(--font-serif);font-size:1.1rem;margin:0 0 4px;">Instant chat answers</h3>'
                ...
                + CHAT_FAQ_ORDER.map(which => {
                    ...
                    const val = (siteContent[f.key] != null && siteContent[f.key] !== '') ? siteContent[f.key] : '';
                    return `<div style="margin-bottom:14px;"><label style="font-size:0.78rem;color:var(--text-muted);display:block;margin-bottom:6px;">${escapeHtml(f.q)}</label>`
        ...
        function toggleArchivedMessages() { __msgShowArchived = !__msgShowArchived; loadAdminMessages(); }
        ...
        async function openMessageThread(threadId) {
            ...
            try {
                ...
                const archBtn = document.getElementById('msg-archive-btn'); if (archBtn) archBtn.textContent = __msgThreadArchived ? 'Unarchive' : 'Archive';
                ...
                if (thread) { thread.innerHTML = chatBubbles(r.messages || [], 'admin'); thread.scrollTop = thread.scrollHeight; }
            } catch (e) { if (thread) thread.innerHTML = `<p class="chat-empty">Couldn't load this thread.</p>`; }
            loadAdminMessages();   // clear the unread badge now it's been read
        }
        function closeMessagesModal() {
            const m = document.getElementById('messages-modal'); if (m) m.classList.remove('open');
            __msgThreadId = null;
        }
        ...
        async function archiveCurrentThread() {
            ...
            try {
                await apiPost('messages.php', { action: archiving ? 'archive' : 'unarchive', thread_id: __msgThreadId });
                closeMessagesModal();
                loadAdminMessages();
        ...
        async function deleteCurrentThread() {
            ...
            } catch (e) { glassAlert("Couldn't delete: " + e.message); }
        }
        async function adminSendMessage() {
            ...
            try {
                const res = await queueOrPost('messages.php', { action: 'send', thread_id: __msgThreadId, body });
                if (input) input.value = '';
                if (res && res.queued) { toast('Saved offline — your reply will send when you reconnect.'); return; }
        ...
        function collectListRows(wrap, attr) {
            const items = [];
            if (wrap) wrap.querySelectorAll(`[data-${attr}]`).forEach(el => { const v = (el.value || '').replace(/\s+/g, ' ').trim(); if (v) items.push(v); });
            return items;
        }

        // ---- Safety & property list (per cottage) ----
        function accomSafetyList(k) {
            return Array.isArray(siteContent['safety-' + k]) ? siteContent['safety-' + k].slice() : DEFAULT_SAFETY.slice();
        }
        ...
        async function accomSaveSafety(k) {
            ...
            try {
                ...
                if (activeFrontProperty === k) { activePropSafety = items.slice(); renderSafety(k); }
                toast('Safety & property saved.');
            } catch (e) { glassAlert("Couldn't save: " + e.message); }
        }

        // ---- House rules list (per cottage, guest-facing bullets) ----
        const DEFAULT_HOUSE_RULES = ['Please treat the cottage as your own home', 'Let us know of any special requests'];
        function houseRulesList(k) {
            return Array.isArray(siteContent['houserules-' + k]) ? siteContent['houserules-' + k].slice() : DEFAULT_HOUSE_RULES.slice();
        }
        function accomAddHouseRule(k) {
            const wrap = document.getElementById('accom-houserules-rows-' + k);
            if (wrap) wrap.insertAdjacentHTML('beforeend', listRowHtml('hr', '', 'e.g. No smoking indoors'));
        }
        ...
        async function saveOccupancy(k) {
            const num = (id, min) => Math.max(min, parseInt((document.getElementById(id) || {}).value, 10) || 0);
            ...
            try {
                ...
                if (activeFrontProperty === k) { try { renderHouseRules(k); } catch (e) {} try { applyOccupancyToForm(k); } catch (e) {} }
            ...
            switch (sec) {
                ...
                case 'house': return `
                    ...
                    <div style="margin-top:6px;"><label style="font-size:0.78rem;color:var(--text-muted);display:block;margin-bottom:8px;">Allowed arrival days <span style="opacity:0.7;">(none ticked = any day)</span></label>
                        <div class="arrival-days">${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((d,di) =>
                    ...
                    ${(() => { const o = occupancyLimits[k] || { maxAdults: 2, maxChildren: 0, maxTotal: 2 }; return `
                    ...
                    <div id="accom-houserules-rows-${k}">${houseRulesList(k).map(s => listRowHtml('hr', s, 'e.g. No smoking indoors')).join('')}</div>
                    <div style="display:flex;gap:10px;margin-top:8px;">
                ...
                case 'safety': return `
                    ...
                    <div id="accom-safety-rows-${k}">${accomSafetyList(k).map(s => listRowHtml('sf', s, 'e.g. Smoke alarm')).join('')}</div>
                    <div style="display:flex;gap:10px;margin-top:8px;">
                ...
                case 'welcome': {
                    const secs = Array.isArray(adminPrivateContent['welcome-'+k]) ? adminPrivateContent['welcome-'+k] : [];
                    return `
        ...
        function faqBlockHtml(propKey) {
            const faqs = Array.isArray(siteContent['faqs-' + propKey]) ? siteContent['faqs-' + propKey] : [];
        ...
        function toggleFaq(id) {
            ...
            if (!ans) return;
            if (item.classList.contains('open')) { item.classList.remove('open'); ans.style.maxHeight = '0'; }
        ...
        function allReviews() {
            ...
            const curated = (Array.isArray(siteContent.reviews) ? siteContent.reviews : [])
                .filter(r => r && (r.text || '').toString().trim());
        ...
        function renderReviews() {
            try { renderCardRatings(); } catch (e) {}   // social proof on the cottage cards (runs even with no reviews)
        ...
        function renderPropReviews(propKey) {
            ...
            const avg = list.reduce((s, r) => s + Math.max(1, Math.min(5, parseInt(r.stars) || 5)), 0) / count;
        ...
        const IC_MEDAL = '<svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="9" r="5"/><path d="M9 13.5L8 21l4-2 4 2-1-7.5"/></svg>';
        function renderPropStats(propKey) {
            const el = document.getElementById('prop-stats');
            if (!el) return;
            const list = allReviews().filter(r => r.prop === propKey);
            const count = list.length;
            const avg = count ? list.reduce((s, r) => s + Math.max(1, Math.min(5, parseInt(r.stars) || 5)), 0) / count : 0;
        ...
        function updateBookBar() {
            ...
            const adults = Math.max(1, parseInt((document.getElementById('enq-adults') || {}).value, 10) || 2);
        ...
        function openEnquireModal() {
            ...
            if (rateEl) {
                ...
                const avg = cnt ? list.reduce((s, r) => s + Math.max(1, Math.min(5, parseInt(r.stars) || 5)), 0) / cnt : 0;
        ...
        function enquireContinue() {
            ...
            const ci = (document.getElementById('enq-checkin') || {}).value;
            const co = (document.getElementById('enq-checkout') || {}).value;
            if (!ci || !co) { setEnqMsg('review', 'Please choose your stay dates.'); return; }
            if (co <= ci) { setEnqMsg('review', 'Your check-out date must be after your check-in date.'); return; }
            const ruleErr = checkBookingRules(activeFrontProperty, ci, co);
            if (ruleErr) { setEnqMsg('review', ruleErr); return; }
            ...
            if (occErr) { setEnqMsg('review', occErr); return; }
            const r = document.getElementById('enquire-step-review');
            const d = document.getElementById('enquire-step-details');
        ...
        function reviewRowHtml(r) {
            ...
            const propOpts = ['<option value="">(no cottage)</option>'].concat(
                Object.keys(propertyMeta).map(k => `<option value="${k}" ${r.prop === k ? 'selected' : ''}>${propertyMeta[k].name}</option>`)).join('');
            const starOpts = [5,4,3].map(n => `<option value="${n}" ${parseInt(r.stars) === n ? 'selected' : ''}>${'★'.repeat(n)}</option>`).join('');
            const srcOpts = ['', 'Airbnb', 'Vrbo', 'Booking.com', 'Google', 'Email', 'Guestbook']
                .map(s => `<option value="${s}" ${(r.source || '') === s ? 'selected' : ''}>${s || '(no source)'}</option>`).join('');
        ...
        async function saveFaqs(propKey) {
            ...
            for (const row of wrap.querySelectorAll('.faq-row')) {
                const get = sel => { const el = row.querySelector(`[data-fq="${sel}"]`); return el ? el.value.trim() : ''; };
                const q = get('q'), a = get('a'), icon = get('icon');
                if (!q && !a) continue;               // empty row — skip
                if (!q || !a) { glassAlert('Each entry needs both a question and an answer.'); return; }
                faqs.push({ icon, q, a });
            }
            try {
        ...
        async function saveWelcome(propKey) {
            ...
            for (const row of wrap.querySelectorAll('.welcome-row')) {
                const get = sel => { const el = row.querySelector(`[data-wb="${sel}"]`); return el ? el.value.trim() : ''; };
                const title = get('title'), body = get('body');
                if (!title && !body) continue;            // empty row — skip
                if (!title || !body) { glassAlert('Each section needs both a title and some details.'); return; }
        ...
        async function loadWaitlist() {
            ...
            wrap.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">Loading…</p>`;
            let rows = [];
            try { const r = await apiGet('waitlist.php'); rows = r.waitlist || []; }
            catch (e) { wrap.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">Couldn't load the waitlist${e && e.message ? ' (' + escapeHtml(e.message) + ')' : ''}.</p>`; return; }
            if (!rows.length) { wrap.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">No one on the waitlist yet.</p>`; return; }
            wrap.innerHTML = rows.map(w => {
                const name = (propertyMeta[w.prop_key] || {}).name || w.prop_key;
                const dates = (w.check_in && w.check_out) ? `${w.check_in} → ${w.check_out}` : 'Any dates';
                const notified = w.notified_at ? `<span style="color:#4CAF50;">Notified ${escapeHtml(String(w.notified_at).slice(0,10))}</span>` : '<span style="color:var(--text-muted);">Waiting</span>';
        ...
        async function loadNewsletter() {
            ...
            const sendMsg = document.getElementById('nl-send-msg'); if (sendMsg) sendMsg.textContent = '';
            if (!stats) return;
            stats.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">Loading…</p>`;
            let r;
            try { r = await apiGet('newsletter.php'); }
            catch (e) { stats.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">Couldn't load subscribers${e && e.message ? ' (' + escapeHtml(e.message) + ')' : ''}.</p>`; return; }
            const active = r.active || 0, total = r.total || 0;
        ...
        async function runMigrations() {
            ...
            try {
                ...
                if (out) {
                    ...
                    out.innerHTML = list.length ? list.map(m => {
                    ...
                    }).join('') : '<div style="color:var(--text-muted);">No migration files found.</div>';
                }
        ...
        async function loadDiagnostics() {
            ...
            const msg = document.getElementById('diag-msg'); if (msg) msg.textContent = '';
            if (!body) return;
            body.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">Running checks…</p>`;
            let r;
            try { r = await apiPost('diagnostics.php', { action: 'run' }); }
            catch (e) { body.innerHTML = `<p style="font-size:0.85rem;color:#E57373;">Couldn't run checks: ${escapeHtml(e.message || '')}</p>`; return; }
            const checks = r.checks || [], s = r.summary || {};
            const dot = st => st === 'ok' ? '#4CAF50' : (st === 'warn' ? '#FFB74D' : '#E57373');
            const word = st => st === 'ok' ? 'OK' : (st === 'warn' ? 'Optional' : 'Action needed');
            // Group by category, preserving order of first appearance.
            const cats = [];
            checks.forEach(c => { if (!cats.includes(c.category)) cats.push(c.category); });
            ...
            body.innerHTML = summary + cats.map(cat => `
        ...
        async function sendBroadcast() {
            ...
            const show = (t, ok) => { if (msg) { msg.style.color = ok ? '#4CAF50' : '#E57373'; msg.textContent = t; } };
            if (!subject || !bodyText) { show('A subject and a message are both required.', false); return; }
            ...
            try {
                ...
                show(`Sent to ${r.sent || 0} subscriber${(r.sent === 1) ? '' : 's'}${r.failed ? ` (${r.failed} failed)` : ''}.`, true);
                if (r.sent) { if (subEl) subEl.value = ''; if (bodyEl) bodyEl.value = ''; }
            } catch (e) { show(e.message || "Couldn't send the broadcast.", false); }
        }
        async function notifyWaitlist(id) {
            if (!await glassConfirm('Email this guest that dates may now be available?')) return;
            try { await apiPost('waitlist.php', { action: 'notify', id }); toast('Guest emailed.'); loadWaitlist(); }
            catch (e) { glassAlert("Couldn't send: " + e.message); }
        }
        async function deleteWaitlist(id) {
            if (!await glassConfirm('Remove this waitlist entry?')) return;
            try { await apiPost('waitlist.php', { action: 'delete', id }); loadWaitlist(); }
            catch (e) { glassAlert("Couldn't remove: " + e.message); }
        }
        ...
        async function loadGuestPhotosAdmin() {
            ...
            wrap.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">Loading…</p>`;
            let rows = [];
            try { const r = await apiPost('photos.php', { action: 'list_admin' }); rows = r.photos || []; }
            catch (e) { wrap.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">Couldn't load (run migrate.php?): ${escapeHtml(e.message || '')}</p>`; return; }
            if (!rows.length) { wrap.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">No guest photos yet. They'll appear here when guests share photos from My Bookings.</p>`; return; }
            wrap.innerHTML = `<div class="guest-photo-grid" style="grid-template-columns:repeat(auto-fill,minmax(180px,1fr));">${rows.map(p => {
        ...
        function initGoogleReviewUrl() {
            ...
            const msg = document.getElementById('google-review-url-msg'); if (msg) msg.textContent = '';
        }
        async function saveGoogleReviewUrl() {
            ...
            if (msg) { msg.style.color = '#4CAF50'; msg.textContent = val ? 'Saved ✓' : 'Cleared.'; }
        }
        async function loadGuestReviewModeration() {
            ...
            if (!wrap) return;
            let rows = [];
            try { const r = await apiPost('reviews.php', { action: 'list_admin' }); rows = r.reviews || []; }
            catch (e) { wrap.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">Couldn't load (run migration-guest-reviews.sql?): ${escapeHtml(e.message)}</p>`; return; }
            if (!rows.length) { wrap.innerHTML = `<p style="font-size:0.85rem;color:var(--text-muted);">No guest reviews yet.</p>`; return; }
        ...
        async function deleteGuestReview(id) {
            ...
            try {
                ...
                await loadGuestReviewModeration();
                await loadPublicReviews(); renderReviews();
            } catch (e) { glassAlert("Couldn't delete: " + e.message); }
        }
        ...
        function quickAddReview() {
            ...
            if (!text) { glassAlert('Please paste the review text first.'); return; }
            const r = {
        ...
        async function saveReviews() {
            ...
            for (const row of wrap.querySelectorAll('.review-row')) {
                const get = f => { const el = row.querySelector(`[data-rf="${f}"]`); return el ? el.value : ''; };
                const text = get('text').trim();
                if (!text) continue;   // empty review — skip
                reviews.push({ name: get('name').trim(), stars: parseInt(get('stars')) || 5, text, prop: get('prop'), source: get('source') });
            }
            try {
                ...
                renderReviews();
                toast('Reviews saved.');
            } catch (e) { glassAlert("Couldn't save reviews: " + e.message); }
        }
        ...
        async function saveSeasons(k) {
            ...
            for (const row of wrap.querySelectorAll('.season-row')) {
                const get = f => { const el = row.querySelector(`[data-sf="${f}"]`); return el ? el.value.trim() : ''; };
                const label = get('label'), start = get('start'), end = get('end'), rate = parseFloat(get('rate'));
                if (!start && !end && !rate && !label) continue;   // fully empty row — skip
                if (!start || !end || !(rate > 0)) { glassAlert('Each season needs a start date, an end date and a couple rate above £0.'); return; }
            ...
            try {
                ...
                propertySeasons[k] = seasons.map(s => ({ label: s.label, start_date: s.start, end_date: s.end, couple_rate: s.rate }));
                renderCardPrices(); updatePropPriceHeading();
                toast('Seasonal rates saved.');
            } catch (e) { glassAlert("Couldn't save seasons: " + e.message); }
        }
        ...
        async function saveRateField(propKey, field, value) {
            ...
            } catch (e) { glassAlert("Couldn't save rate: " + e.message); }
        }
        ...
        function toggleArrivalDay(propKey, dayIndex, checked) {
            if (!propertyRates[propKey]) propertyRates[propKey] = Object.assign({}, defaultRates[propKey]);
            let days = Array.isArray(propertyRates[propKey].arrivalDays) ? propertyRates[propKey].arrivalDays.slice() : [];
            if (checked) { if (!days.includes(dayIndex)) days.push(dayIndex); }
        ...
        function saveRules(propKey) {
            ...
            try { localStorage.setItem('rules-' + propKey, JSON.stringify(rules)); } catch (e) {}
        ...
        function todayDashed() { const t = ukNowParts(); return `${t.y}-${String(t.m).padStart(2, '0')}-${String(t.d).padStart(2, '0')}`; }
        ...
        function glassDialog(opts) {
            const run = () => new Promise(resolve => {
                ...
                if (!o) { resolve(opts.type === 'prompt' ? null : (opts.type !== 'confirm')); return; }
                ...
                setTimeout(() => { (opts.type === 'prompt' ? inp : document.getElementById('glass-dialog-ok')).focus(); }, 60);
            });
            const p = __glassDlgQueue.then(run);
            __glassDlgQueue = p.then(() => {}, () => {});
            return p;
        }
        function glassDialogResolve(ok) { if (__glassDlgResolve) __glassDlgResolve(ok); }
        function glassAlert(message) { return glassDialog({ type: 'alert', message }); }
        function glassConfirm(message) { return glassDialog({ type: 'confirm', message }); }
        function glassPrompt(message, def, opts) { return glassDialog({ type: 'prompt', message, def, password: !!(opts && opts.password) }); }
        ...
        function toast(message, type) {
            ...
            if (!stack) { stack = document.createElement('div'); stack.id = 'app-toasts'; stack.className = 'toast-stack'; document.body.appendChild(stack); }
            const ok = type !== 'error';
            const icon = ok
            ...
            let gone = false;
            const remove = () => { if (gone) return; gone = true; el.classList.add('toast-out'); setTimeout(() => el.remove(), 360); };
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Enter') { e.preventDefault(); glassDialogResolve(true); }
            if (e.key === 'Escape') { e.preventDefault(); glassDialogResolve(false); }
        });
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Escape') {
                ...
                if (closer) closer(); else m.classList.remove('open');
            } else if (m.id === 'lightbox' && (e.key === 'ArrowLeft' || e.key === 'ArrowRight')) {
                e.preventDefault();
                lightboxNav(e.key === 'ArrowLeft' ? -1 : 1);
            } else if (e.key === 'Tab') {
                // Keep keyboard focus inside the open dialog.
                const focusable = m.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])');
        ...
        async function loadAvailability(propKey) {
            ...
            } catch (e) { /* keep whatever we had; server still enforces */ }
        }
        ...
        function renderAvailCal() {
            ...
            if (!availCalMonth) availCalMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
            const year = availCalMonth.getFullYear(), month = availCalMonth.getMonth();
            ...
            const rate = propertyRates[activeFrontProperty] || defaultRates[activeFrontProperty] || { coupleRate: 0 };
        ...
        function dpParse(str) { if (!str) return null; const [y,m,d] = str.split('-').map(Number); return new Date(y, m-1, d); }
        function dpToday0() { const u = ukNowParts(); const t = new Date(u.y, u.m - 1, u.d); t.setHours(0,0,0,0); return t; }
        function dpPretty(str) {
            const d = dpParse(str); if (!d) return '';
        ...
        function closeDatePicker() { document.getElementById('date-picker').classList.remove('open'); }
        ...
        function renderDatePicker() {
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                else if (dpMode === 'search') clickable = true;                           // hero search: any future date
                else if (!pickingEnd) clickable = !booked;
                else if (ds <= dpState.start) clickable = !booked;                       // restart selection
                ...
                if (dpState.start && dpState.end && ds > dpState.start && ds < dpState.end) classes.push('dp-in-range');
        ...
        function dpPick(ds) {
            ...
            if (!dpState.start || (dpState.start && dpState.end)) {
                dpState.start = ds; dpState.end = null;
            } else {
                if (ds <= dpState.start) {
        ...
        function dpDone() {
            if (dpMode === 'search') {
                ...
                if (disp) disp.innerText = (heroSearch.checkin && heroSearch.checkout)
                    ? `${dpPretty(heroSearch.checkin)} → ${dpPretty(heroSearch.checkout)}`
                    : (heroSearch.checkin ? `${dpPretty(heroSearch.checkin)} — pick check-out` : 'Add your dates');
                closeDatePicker();
                return;
            }
        ...
        function hsRestore() {
            let s; try { s = JSON.parse(localStorage.getItem('chb-search') || 'null'); } catch (e) { s = null; }
            if (!s) return;
            ...
            const disp = document.getElementById('hs-dates');
            if (disp && heroSearch.checkin) disp.innerText = heroSearch.checkout
                ? `${dpPretty(heroSearch.checkin)} → ${dpPretty(heroSearch.checkout)}`
                : `${dpPretty(heroSearch.checkin)} — pick check-out`;
            if (heroSearch.cottage && heroSearch.cottage !== 'any') {
                const opt = document.querySelector('#hs-cottage-menu .hs-opt[data-key="' + heroSearch.cottage + '"]');
                const lbl = document.getElementById('hs-cottage-label');
                if (opt && lbl) lbl.innerText = opt.textContent.trim();
                document.querySelectorAll('#hs-cottage-menu .hs-opt').forEach(o => o.classList.toggle('is-sel', o.getAttribute('data-key') === heroSearch.cottage));
            }
        }
        ...
        function hsSetCottage(v) { heroSearch.cottage = v; hsMaybeRerun(); }
        ...
        function chooseCottage(key, label) {
            ...
            document.querySelectorAll('#hs-cottage-menu .hs-opt').forEach(o => o.classList.toggle('is-sel', o.getAttribute('data-key') === key));
            closeCottageMenu();
            hsMaybeRerun();
        }
        function hsSetFlex(n) {
            heroSearch.flex = n;
            document.querySelectorAll('.hs-chip').forEach(c => c.classList.toggle('is-on', String(n) === c.getAttribute('data-flex')));
            hsMaybeRerun();
        }
        ...
        function findAvailability(key, ci, co, adults, children, flex) {
            ...
            if (adults > lim.maxAdults || children > lim.maxChildren || (adults + children) > lim.maxTotal) return { fits: false };
            ...
            for (const off of flexOffsets(flex)) {
                ...
                if (ruleErr) { if (off === 0) reason = ruleErr; continue; }
        ...
        async function runHeroSearch() {
            const msg = document.getElementById('hs-msg');
            const setMsg = (t, ok) => { if (msg) { msg.innerText = t || ''; msg.style.color = ok ? 'var(--text-muted)' : '#FFB74D'; } };
            const ci = heroSearch.checkin, co = heroSearch.checkout;
            if (!ci || !co) { setMsg('Please choose your check-in and check-out dates.'); return; }
            if (co <= ci) { setMsg('Your check-out date must be after check-in.'); return; }
        ...
        function renderHeroResults(results, tooSmall) {
            ...
            const propImg = (key) => (propertyContent[key] && propertyContent[key].images && propertyContent[key].images[0]) || '';
            const party = heroSearch.adults + heroSearch.children;
            const flexNote = heroSearch.flex ? ` (±${heroSearch.flex} day${heroSearch.flex === 1 ? '' : 's'})` : '';
            const sub = (t) => `<h3 style="grid-column:1/-1;text-align:center;font-family:var(--font-serif);font-weight:400;margin:22px 0 2px;font-size:1.25rem;">${escapeHtml(t)}</h3>`;
            ...
            const card = (key, r) => {
                ...
                const banner = r.offset ? `<div class="hs-banner">Your dates (${reqRange}) aren't free here — these are the closest available.</div>` : '';
            ...
            const noneMsg = (txt) => `<div class="glass-panel" style="grid-column:1/-1;text-align:center;padding:28px;">
            ...
            if (filter === 'any') {
                title.innerText = availKeys.length ? `Available for your dates${flexNote}` : `No cottages free for those dates${flexNote}`;
                html += availKeys.map(k => card(k, results[k])).join('');
                html += fitKeys.filter(k => !results[k].available).map(k => unavailCard(k, results[k].reason)).join('');
                if (!fitKeys.length) html = noneMsg(`Sorry, none of our cottages can host ${party} guest${party === 1 ? '' : 's'}.`);
                else if (tooSmall > 0) html += `<p style="grid-column:1/-1;text-align:center;font-size:0.8rem;color:var(--text-muted);margin-top:6px;">${tooSmall} cottage${tooSmall === 1 ? ' was' : 's were'} hidden — too small for ${party} guests.</p>`;
            } else {
                const X = filter, xName = propertyMeta[X] ? propertyMeta[X].name : X;
                const xr = results[X];
                const others = availKeys.filter(k => k !== X);
                if (xr && xr.available) {
                    title.innerText = `${xName} is available${flexNote}`;
                    html += card(X, xr);
                    if (others.length) { html += sub('Other cottages free for your dates'); html += others.map(k => card(k, results[k])).join(''); }
                } else {
                    ...
                    if (others.length) {
                        ...
                        html += noneMsg('No other cottage is free for those dates either — try different dates or widen the flexibility.');
                    }
                }
            }
        ...
        function fluidSwap(el) {
            if (!el) return;
            el.classList.remove('fluid-swap'); void el.offsetWidth; el.classList.add('fluid-swap');
        }
        ...
        function startBooking(key, ci, co) {
            ...
            const target = document.getElementById('enq-date-trigger') || document.getElementById('enq-name');
            if (target) setTimeout(() => target.scrollIntoView({ behavior: 'smooth', block: 'center' }), 160);
        }
        ...
        function rebookCottage(propKey) {
            try { openProperty(propKey); } catch (e) {}   // openProperty navigates to the cottage page itself
            setTimeout(() => { try { openEnquireModal(); } catch (e) {} }, 220);
        }
        ...
        function renderOwnerSummary() {
            ...
            let arrivals30 = 0, received = 0, outstanding = 0, unpaidUpcoming = 0;
            ...
            Object.keys(dbBlocks).forEach(propKey => {
                (dbBlocks[propKey] || []).forEach(bl => {
                    ...
                    if (ci) { const ds = formatDashed(ci); if (ds >= todayStr && ds <= in30) arrivals30++; }
                });
            });
            ...
            const paidFrac = (received + outstanding) > 0 ? received / (received + outstanding) : (received > 0 ? 1 : 0);
            el.innerHTML = `
        ...
        function osDonut(pct, color) {
            ...
            const R = 26, C = 2 * Math.PI * R, dash = (C * pct / 100).toFixed(1);
        ...
        let __homeVisits = null, __homeVisitsAt = 0;
        async function refreshHomeVisits() {
            const v = document.getElementById('os-visits');
            if (!v) return;
            if (!(__homeVisits && (Date.now() - __homeVisitsAt < 60000))) {
                try { __homeVisits = await apiGet('track.php?action=summary&days=7'); __homeVisitsAt = Date.now(); }
                catch (e) { __homeVisits = null; }
            }
            ...
            if (__homeVisits) {
                ...
                const daily = Array.isArray(__homeVisits.daily) ? __homeVisits.daily.map(d => d.views || 0) : [];
        ...
        function osMiniDonut(pct, color) {
            ...
            const R = 15, C = 2 * Math.PI * R, dash = (C * pct / 100).toFixed(1);
        ...
        function moneyShort(v) { v = +v || 0; return v >= 1000 ? '£' + (v / 1000).toFixed(v >= 10000 ? 0 : 1) + 'k' : '£' + Math.round(v); }
        // Vertical bars: items = [{label, short, value}].
        function osVBars(items, fmt) {
            if (!items || !items.length) return '';
            const peak = Math.max(1, ...items.map(i => i.value || 0));
            return `<div style="display:flex;align-items:flex-end;gap:8px;height:140px;margin:12px 0 2px;">` + items.map(i => {
        ...
        function cottageMonthOccupancy() {
            ...
            const sets = {}; const out = {};
            Object.keys(propertyMeta).forEach(k => { sets[k] = new Set(); out[k] = { nights: 0, total: days, pct: 0 }; });
            const add = (k, ci, co) => {
                if (!sets[k]) return;
                let d = dpParse(ci), e = dpParse(co); if (!d || !e) return;
                for (; d < e; d.setDate(d.getDate() + 1)) { const ds = formatDashed(d); if (ds >= mStart && ds <= mEnd) sets[k].add(ds); }
            };
            Object.keys(dbBookings).forEach(k => (dbBookings[k] || []).forEach(b => add(k, b.checkIn, b.checkOut)));
            Object.keys(dbBlocks || {}).forEach(k => (dbBlocks[k] || []).forEach(bl => add(k, bl.checkIn, bl.checkOut)));
            Object.keys(out).forEach(k => { out[k].nights = sets[k].size; out[k].pct = Math.round(sets[k].size / days * 100); });
            return out;
        }
            ...
            for(let d = 1; d <= daysInMonth; d++) {
                ...
                Object.keys(dbBookings).forEach(propKey => {
                    ...
                    const payColor = paymentMeta[dayData.booking.payment]
                        ? paymentMeta[dayData.booking.payment].dot : '#888';
                    ...
                    if (dayData.status === 'check-in') {
                        ...
                        bar.onclick = (e) => { e.stopPropagation(); showDetails(propKey, dayData.booking); };
                        ...
                        bar.onclick = (e) => { e.stopPropagation(); showDetails(propKey, dayData.booking, dayData.nextBooking); };
                    } else { // booked (mid-stay)
                        bar.innerHTML = `${dot}<span class="bb-code">${short}</span><span class="bb-name"> · ${escapeHtml(firstName)}</span>`;
                        bar.onclick = (e) => { e.stopPropagation(); showDetails(propKey, dayData.booking); };
                    }
        ...
        function showDetails(propKey, booking1, booking2 = null) {
            ...
            if (squareAdminEnabled) {
                try { if (booking1 && booking1.email) loadBookingPayments(booking1.id); } catch (e) {}
                try { if (booking2 && booking2.email) loadBookingPayments(booking2.id); } catch (e) {}
            }
        ...
        function buildDetailHtml(propKey, b, titlePrefix = null) {
            const meta = propertyMeta[propKey];
            const title = titlePrefix ? `<h4 style="color: var(--text-muted); margin-bottom: 10px; font-size: 0.85rem; text-transform: uppercase;">${titlePrefix}</h4>` : '';
            const inTime = b.checkInTime ? ` · ${b.checkInTime}` : '';
            const outTime = b.checkOutTime ? ` · ${b.checkOutTime}` : '';
            const emailVal = b.email ? `<a href="mailto:${escapeHtml(b.email)}" style="color: var(--text-light);">${escapeHtml(b.email)}</a>` : '<span style="color: var(--text-muted);">—</span>';
            const phoneVal = b.phone ? `<a href="tel:${escapeHtml(b.phone)}" style="color: var(--text-light);">${escapeHtml(b.phone)}</a>` : '<span style="color: var(--text-muted);">—</span>';
            ...
            const ps = paymentSummary(propKey, b);
            const depositRows = ps.deposit > 0 ? `
            ...
            return `
                ...
                ${b.email ? `<div style="margin-top: 12px; max-width: 320px;">
        ...
        function injectSeoSchema() {
            try {
                ...
                if (reviews.length) {
                    ...
                    biz.aggregateRating = {
                        '@type': 'AggregateRating', 'ratingValue': avg.toFixed(1),
                        'reviewCount': String(reviews.length), 'bestRating': '5', 'worstRating': '1'
                    };
                    biz.review = reviews.slice(0, 12).map(r => ({
                        '@type': 'Review',
                        'author': { '@type': 'Person', 'name': (r.name || 'A guest') },
                        'reviewRating': { '@type': 'Rating', 'ratingValue': String(clamp(r.stars)), 'bestRating': '5', 'worstRating': '1' },
                        'reviewBody': (r.text || '')
                    }));
                }
                ...
                if (faqEntities.length) graph.push({ '@type': 'FAQPage', '@id': BASE + '#faq', 'mainEntity': faqEntities });

                if (!graph.length) return;
                let el = document.getElementById('seo-dynamic');
                if (!el) { el = document.createElement('script'); el.type = 'application/ld+json'; el.id = 'seo-dynamic'; document.head.appendChild(el); }
                el.textContent = JSON.stringify({ '@context': 'https://schema.org', '@graph': graph });
            } catch (e) { /* SEO enrichment must never break the page */ }
        }
        ...
        function syncCottageUrl(propKey) {
            ...
            if (!__suppressRouteSync) {
                try { (location.pathname === url ? history.replaceState : history.pushState).call(history, { cottage: propKey }, '', url); } catch (e) {}
            }
        ...
        function openProperty(propKey) {
            ...
            document.getElementById('prop-price-tagline').setAttribute('data-edit-text', `${propKey}-tagline`);
            // Things-to-know + location: per-cottage editable text
            const setEdit = (id, key) => { const el = document.getElementById(id); if (el) el.setAttribute('data-edit-text', key); };
            ...
            if (cancEl) { cancEl.removeAttribute('data-edit-text'); cancEl.contentEditable = 'false'; }
            ...
            try { renderPropReviews(propKey); } catch (e) {}
            try { renderPropStats(propKey); } catch (e) {}
            try { renderHouseRules(propKey); } catch (e) {}
            try { renderLocalGuide(propKey); } catch (e) {}
            try { renderGuestPhotos(propKey); } catch (e) {}
            try { renderTides(); } catch (e) {}
            try { renderLocationMap(propKey); } catch (e) {}
            try { renderHost(); } catch (e) {}
            try { renderPropFeatures(propKey); } catch (e) {}
            try { updateBookBar(); } catch (e) {}

            nav('view-21a');
            try { syncCottageUrl(propKey); } catch (e) {}   // clean URL + per-cottage SEO snippet
            try { setupPropDescClamp(); } catch (e) {}   // clamp long descriptions once the view is visible
            try { placeAvailCalendar(); } catch (e) {}    // desktop: tuck calendar above "Enquire now"
        }
        ...
        function updateEnquiryPrice() {
            ...
            const r = propertyRates[activeFrontProperty] || defaultRates[activeFrontProperty];
            const extras = [];
            if (p.extraAdults > 0) extras.push(`${p.extraAdults} extra adult${p.extraAdults === 1 ? '' : 's'}`);
        ...
        function applyOccupancyToForm(propKey) {
            ...
            const ac = document.getElementById('enq-adults-count'); if (ac && aEl) ac.textContent = aEl.value;
            const cc = document.getElementById('enq-children-count'); if (cc && cEl) cc.textContent = cEl.value;
        ...
        function bookingRulesHint(propKey) {
            ...
            if (days.length > 0) {
                const names = days.slice().sort((a,b)=>a-b).map(i => DAY_NAMES[i]);
        ...
        async function submitEnquiry(propKey) {
            ...
            if (ruleErr) { setEnqMsg('details', ruleErr); return; }
            const occErr = checkOccupancy(propKey, adults, children);
            if (occErr) { setEnqMsg('details', occErr); return; }
            const termsBox = document.getElementById('enq-terms');
            if (!termsBox || !termsBox.checked) {
                setEnqMsg('details', "Please read and accept the Booking Terms & Conditions before sending your enquiry.");
                return;
            }
        ...
        async function declineEnquiry(enqId) {
            ...
            try {
                ...
                await loadData();
                renderInbox();
            } catch (e) { glassAlert("Couldn't decline: " + e.message); }
        }
        ...
        async function approveEnquiry(enqId) {
            ...
            if (hasDateClash(enq.propKey, enq.checkIn, enq.checkOut)) {
                if (!await glassConfirm(`Heads up: these dates clash with an existing booking or an imported Airbnb/Vrbo block at ${propertyMeta[enq.propKey].name}. Approve anyway?`)) return;
            }
            try {
        ...
        function setModalFields(f) {
            ...
            if (depEl) depEl.value = (f.agreedPrice && f.agreedPrice.damagesDeposit != null) ? f.agreedPrice.damagesDeposit
                                    : (f.damagesDeposit != null ? f.damagesDeposit : '');
        ...
        function updateModalPrice() {
            ...
            const children = Math.max(0, parseInt(document.getElementById('modal-children').value, 10) || 0);
            if (!checkIn || !checkOut || checkOut <= checkIn) {
        ...
        function openEditEnquiry(enqId) {
            ...
            setModalFields({
                propKey: enq.propKey, name: enq.name, email: enq.email, phone: enq.phone, address: enq.address, postcode: enq.postcode,
                checkIn: enq.checkIn, checkOut: enq.checkOut,
                checkInTime: enq.checkInTime, checkOutTime: enq.checkOutTime,
                adults: enq.adults, children: enq.children, notes: enq.message
            });
        ...
        function openEditBooking(bookingId) {
            ...
            setModalFields({
                propKey: loc.propKey, name: b.name, email: b.email, phone: b.phone, address: b.address, postcode: b.postcode,
                checkIn: b.checkIn, checkOut: b.checkOut,
                checkInTime: b.checkInTime, checkOutTime: b.checkOutTime,
                adults: b.adults, children: b.children, notes: b.notes, payment: b.payment
            });
            togglePaymentField(true);
            openModal();
        }
        ...
        async function saveModal() {
            ...
            const depEl = document.getElementById('modal-damages-deposit');
            const damagesDeposit = depEl && depEl.value !== '' ? Math.max(0, parseFloat(depEl.value) || 0) : null;
            const ovEl = document.getElementById('modal-price-override');
            // Empty string = explicitly clear the override; a value = set it.
            const priceOverride = ovEl ? ovEl.value.trim() : '';
            const errBox = document.getElementById('modal-error');
            const showErr = (m) => { errBox.innerText = m; errBox.style.display = 'block'; };

            if (!name || !checkIn || !checkOut) { showErr('Name and both dates are required.'); return; }
            if (checkOut <= checkIn) { showErr('Check-out must be after check-in.'); return; }

            // Occupancy limit — owner can override with confirmation (e.g. a cot, an exception)
            const occErr = checkOccupancy(propKey, adults, children);
            if (occErr) {
                if (!await glassConfirm(occErr + '\n\nThis is over the normal limit for this property. Save anyway?')) {
                    showErr(occErr);
                    return;
                }
            }
            ...
            if (hasDateClash(propKey, checkIn, checkOut, mode === 'booking' ? id : null)) {
                if (!await glassConfirm(`These dates clash with an existing booking or an imported Airbnb/Vrbo block at ${propertyMeta[propKey].name}. Save anyway?`)) return;
            }
            ...
            if (payment === 'deposit' || payment === 'paid') {
                ...
                const existingDate = (existing && existing.paymentDate) ? existing.paymentDate : todayDashed();
                const d = await glassPrompt('Payment date (YYYY-MM-DD):', existingDate);
                if (d === null) return;
                if (!/^\d{4}-\d{2}-\d{2}$/.test(d.trim())) { showErr('A valid payment date (YYYY-MM-DD) is required.'); return; }
                paymentDate = d.trim();
                const m = await glassPrompt('Payment method (Card / Bank Transfer / Cash / PayPal / Other) — optional:', (existing && existing.paymentMethod) || '');
                paymentMethod = (m === null) ? '' : m.trim();
            }

            const payload = {
                prop_key: propKey, name, email, phone, address, postcode,
                check_in: checkIn, check_out: checkOut,
                check_in_time: checkInTime, check_out_time: checkOutTime,
                adults, children, notes, payment
            };
            ...
            payload.price_override = priceOverride === '' ? '' : Math.max(0, parseFloat(priceOverride) || 0);
            ...
            try {
                ...
                if (mode === 'add') {
                    ...
                    if (addRes && addRes.clash) {
                        ...
                        addRes = await apiPost('bookings.php', { action: 'add', ...payload, override_clash: true });
                    }
                } else {
                    const loc = findBookingLocation(id);
                    if (!loc) return;
                    ...
                    if (upRes && upRes.clash) {
                        ...
                        await apiPost('bookings.php', { action: 'update', ...payload, override_clash: true });
                    }
                    // Offer to email the guest the updated details (e.g. after a date change)
                    if (payload.email && await glassConfirm(`Booking updated. Email ${payload.email} an updated confirmation?`)) {
                        try { await apiPost('bookings.php', { action: 'send_confirmation', id: payload.id }); toast('Updated confirmation sent.'); }
                        catch (e) { glassAlert("Saved, but the email didn't send: " + e.message); }
                    }
                }
                await loadData();
                closeModal();
                ...
                if (mode === 'add' && addRes && addRes.email && addRes.email.guest) {
                    ...
                    } else if (addRes.email.guest.error && addRes.email.guest.error !== 'Mail disabled' && addRes.email.guest.error !== 'No guest email on file') {
                        glassAlert(`Booking saved, but the confirmation email didn't send (${addRes.email.guest.error}). You can resend it from the booking details.`);
                    }
                }
            } catch (e) { showErr(e.message); }
        }
        ...
        // ============================================================
        const EXPERIENCE_CATEGORIES = ['Beaches & coast', 'Walks & nature', 'Boat trips & wildlife', 'Food & drink', 'Family & kids', 'Days out & attractions', 'Local shops & markets'];
        ...
        async function renderExperiencesView() {
            ...
            if (!grid) return;
            try { const res = await apiGet('experiences.php'); __experiences = (res && res.experiences) || []; }
            catch (e) { __experiences = []; }
            if (!__experiences.length) {
                grid.innerHTML = ''; if (filters) filters.innerHTML = '';
                if (empty) empty.style.display = 'block';
                return;
            }
            if (empty) empty.style.display = 'none';
            if (__expFilter !== 'all' && !__experiences.some(x => x.category === __expFilter)) __expFilter = 'all';
            expBuildFilters();
            expRenderCards();
        }
        function expBuildFilters() {
            const filters = document.getElementById('exp-filters'); if (!filters) return;
            const present = EXPERIENCE_CATEGORIES.filter(c => __experiences.some(x => x.category === c));
            if (present.length < 2) { filters.innerHTML = ''; return; }
            filters.innerHTML = ['all'].concat(present).map(c => {
            ...
            filters.querySelectorAll('.exp-chip').forEach(b => b.addEventListener('click', () => {
                __expFilter = b.dataset.cat; expBuildFilters(); expRenderCards();
            }));
        }
        ...
        function expCardHtml(x) {
            ...
            const dist = x.distance
                ...
                : '';
            const actions = (link || phone || directions) ? `<div class="exp-actions">${link}${phone}${directions}</div>` : '';
        ...
        function openExperienceSuggest() {
            if (!currentGuest) { openGuestArea(); return; }   // sign-in first
            const sel = document.getElementById('exp-s-cat');
            if (sel) sel.innerHTML = '<option value="">— Choose a category —</option>' + EXPERIENCE_CATEGORIES.map(c => `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join('');
            ['exp-s-title', 'exp-s-body', 'exp-s-link', 'exp-s-phone'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
            const msg = document.getElementById('exp-s-msg'); if (msg) msg.style.display = 'none';
            const m = document.getElementById('exp-suggest-modal'); if (m) { m.classList.remove('closing'); m.classList.add('open'); }
        }
        function closeExperienceSuggest() {
            const m = document.getElementById('exp-suggest-modal'); if (!m || !m.classList.contains('open')) return;
            m.classList.add('closing'); setTimeout(() => m.classList.remove('open', 'closing'), 350);
        }
        async function submitExperienceSuggestion() {
            const g = id => (document.getElementById(id) ? document.getElementById(id).value : '');
            const title = g('exp-s-title').trim(), body = g('exp-s-body').trim();
            const category = g('exp-s-cat'), link_url = g('exp-s-link').trim(), phone = g('exp-s-phone').trim();
            const msg = document.getElementById('exp-s-msg');
            const show = (t, ok) => { if (msg) { msg.textContent = t; msg.style.color = ok ? '#4CAF50' : '#E53935'; msg.style.display = 'block'; } };
            const fileEl = document.getElementById('exp-s-photo');
            const file = fileEl && fileEl.files && fileEl.files[0];
            if (title.length < 3) { show('Please add a name.', false); return; }
            if (body.length < 10) { show('Please add a short description.', false); return; }
            if (file && file.size > 8 * 1024 * 1024) { show('Please choose an image 8 MB or smaller.', false); return; }
            try {
                // multipart so an optional photo can ride along (validated server-side).
                const fd = new FormData();
                fd.append('action', 'suggest');
                fd.append('title', title); fd.append('body', body); fd.append('category', category);
                fd.append('link_url', link_url); fd.append('phone', phone);
                if (file) fd.append('image', file);
                const r = await fetch(API_BASE + 'experiences.php', { method: 'POST', credentials: 'same-origin', body: fd });
                const data = await r.json().catch(() => ({}));
            ...
            } catch (e) { show(e.message || 'Could not send — please try again.', false); }
        }

        // ---- Admin: curate + moderate (Settings -> Experiences) ----
        let __expAdmin = [];
        async function refreshExpPendingBadge(known) {
            const badge = document.getElementById('exp-pending-badge'); if (!badge) return;
            let n = known;
            if (n === undefined) {
                try { const r = await apiPost('experiences.php', { action: 'list_admin' }); n = (r.experiences || []).filter(x => x.status === 'pending').length; }
                catch (e) { n = 0; }
            }
        ...
        async function loadExperiencesAdmin() {
            ...
            let rows = [];
            try { const r = await apiPost('experiences.php', { action: 'list_admin' }); rows = r.experiences || []; }
            catch (e) { wrap.innerHTML = `<p style="color:#E57373;font-size:0.9rem;">${escapeHtml(e.message || 'Could not load — has migrate.php been run?')}</p>`; return; }
            __expAdmin = rows;
            const pending = rows.filter(r => r.status === 'pending');
            ...
            html += `<div id="exp-admin-list">` + (published.length ? published.map(expEditHtml).join('') : `<p style="color:var(--text-muted);font-size:0.88rem;">None yet — add your first experience.</p>`) + `</div>`;
            wrap.innerHTML = html;
            refreshExpPendingBadge(pending.length);
        }
        function expPendingHtml(r) {
            const thumb = r.image_url ? `<div class="exp-edit-thumb" style="background-image:url('${escapeHtml(r.image_url)}');margin-bottom:10px;"></div>` : '';
        ...
        function expEditHtml(r) {
            const id = r.id || 0;
            const catOpts = ['<option value="">— Category —</option>'].concat(
                EXPERIENCE_CATEGORIES.map(c => `<option value="${escapeHtml(c)}"${c === r.category ? ' selected' : ''}>${escapeHtml(c)}</option>`)).join('');
        ...
        function expAddNew() {
            const list = document.getElementById('exp-admin-list'); if (!list) return;
            if (list.querySelector('.exp-edit[data-id="0"]')) return;   // one blank at a time
            const blank = { id: 0, title: '', body: '', image_url: '', link_label: '', link_url: '', phone: '', category: '' };
        ...
        function expUpload(id) {
            pickAndUpload('experience', async (url) => {
                const h = document.getElementById('exp-img-' + id); if (h) h.value = url;
                const t = document.getElementById('exp-thumb-' + id); if (t) t.style.backgroundImage = `url('${url}')`;
            });
        }
        async function expSave(id) {
            ...
            const payload = {
                action: 'save', id: id || 0,
                title: g('exp-t-' + id).trim(), body: g('exp-b-' + id).trim(),
                image_url: g('exp-img-' + id), link_label: g('exp-ll-' + id).trim(),
                link_url: g('exp-lu-' + id).trim(), phone: g('exp-p-' + id).trim(), category: g('exp-c-' + id),
                distance: g('exp-d-' + id).trim(), map_query: g('exp-m-' + id).trim()
            };
            if (!payload.title) { glassAlert('Please add a title.'); return; }
            try { await apiPost('experiences.php', payload); await loadExperiencesAdmin(); } catch (e) { glassAlert(e.message); }
        }
        async function expDelete(id) {
            if (!id) { const row = document.querySelector('.exp-edit[data-id="0"]'); if (row) row.remove(); return; }
            if (!confirm('Delete this experience?')) return;
            try { await apiPost('experiences.php', { action: 'delete', id }); await loadExperiencesAdmin(); } catch (e) { glassAlert(e.message); }
        }
        async function expApprove(id) { try { await apiPost('experiences.php', { action: 'approve', id }); await loadExperiencesAdmin(); } catch (e) { glassAlert(e.message); } }
        async function expReject(id) { try { await apiPost('experiences.php', { action: 'reject', id }); await loadExperiencesAdmin(); } catch (e) { glassAlert(e.message); } }
        async function expMove(id, dir) {
            const pub = __expAdmin.filter(r => r.status === 'published').map(r => r.id);
            const i = pub.indexOf(id); if (i < 0) return;
            const j = i + dir; if (j < 0 || j >= pub.length) return;
            const t = pub[i]; pub[i] = pub[j]; pub[j] = t;
            try { await apiPost('experiences.php', { action: 'reorder', ids: pub }); await loadExperiencesAdmin(); } catch (e) { glassAlert(e.message); }
        }
