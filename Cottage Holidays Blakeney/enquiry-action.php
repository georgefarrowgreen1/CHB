...
function ea_page($title, $bodyHtml)
{
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">' .
        ...
        ' — Cottage Holidays Blakeney</title>' .
...
$summary =
    ...
    ' adult' .
    ((int) $e['adults'] === 1 ? '' : 's') .
    ((int) $e['children'] ? ' + ' . (int) $e['children'] . ' child' . ((int) $e['children'] === 1 ? '' : 'ren') : '') .
    '</div>';
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    ea_page(
        ...
        '<h1>' .
            ...
            $note .
            '</p>' .
            '<form method="post" class="row"><input type="hidden" name="id" value="' .
            (int) $id .
            '">' .
            ...
            ($action === 'approve' ? 'Approve booking' : 'Decline enquiry') .
            '</button></form>',
    );
}
