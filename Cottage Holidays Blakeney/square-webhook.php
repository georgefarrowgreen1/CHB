...
if ($key === '' || $url === '') { json_out(['error' => 'Webhook not configured'], 503); }
$expected = base64_encode(hash_hmac('sha256', $url . $raw, $key, true));
if ($sig === '' || !hash_equals($expected, $sig)) { json_out(['error' => 'Invalid signature'], 401); }
...
if (strpos($type, 'payment.') !== 0 || !$payment) { json_out(['ok' => true]); }
...
if (!$bookingId && preg_match('/(\d+)/', $refId, $m)) { $bookingId = (int)$m[1]; }
if (!$bookingId) { json_out(['ok' => true]); }
