...
if ($action === 'status') {
    ...
    json_out([
        ...
        'hero' => [
            'url' => $h['url'],
            'bytes' => $h['bytes'],
            'optimized' => strpos($h['url'], '-opt') !== false || $h['bytes'] < 400 * 1024,
        ],
    ]);
}
...
if ($action === 'optimize') {
    if (!function_exists('imagecreatefromjpeg')) {
        json_out(['error' => 'The GD image library is not available on this host.'], 500);
    }
    $h = hero_current();
    if (!$h) {
        json_out(['error' => "No uploaded hero found (content key 'hero-bg')."], 404);
    }

    $info = @getimagesize($h['path']);
    if ($info === false) {
        json_out(['error' => 'The current hero could not be read as an image.'], 500);
    }
    $type = $info[2];
    $img = $type === IMAGETYPE_PNG ? @imagecreatefrompng($h['path']) : @imagecreatefromjpeg($h['path']);
    if (!$img) {
        json_out(['error' => 'The current hero could not be decoded.'], 500);
    }
    ...
    $dest = __DIR__ . '/' . $name;
    if (!@imagejpeg($img, $dest, 78)) {
        imagedestroy($img);
        json_out(['error' => 'Could not write the optimised hero (check uploads/ permissions).'], 500);
    }
    ...
    try {
        db()
            ->prepare(
                "INSERT INTO content (item_key, item_value) VALUES ('hero-bg', ?)
        ...
        @unlink($dest . '.webp');
        json_out(
            ['error' => 'Optimised image saved but the site could not be pointed at it: ' . $e->getMessage()],
            500,
        );
    }
    ...
    json_out([
        'ok' => true,
        'url' => $name,
        'before_bytes' => $h['bytes'],
        'after_bytes' => filesize($dest),
