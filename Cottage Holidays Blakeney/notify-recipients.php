...
function nr_save($list)
{
    db()
        ->prepare(
            "INSERT INTO content (item_key, item_value) VALUES ('notify-emails', ?)
...
function nr_apply($action, $email, $list, $primary, $max = NOTIFY_MAX)
{
    ...
    if ($action === 'add') {
        ...
        if ($primary !== '' && strtolower($email) === strtolower($primary)) {
            return [
                'list' => $list,
                'changed' => false,
                'error' => "That's already your primary notification address.",
                'code' => 400,
            ];
        }
        foreach ($list as $e) {
            if (strtolower($e) === strtolower($email)) {
                return ['list' => $list, 'changed' => false, 'error' => null, 'code' => 200];
            }
        } // already there
        if (count($list) >= $max) {
            return [
                'list' => $list,
                'changed' => false,
                'error' => 'You can add up to ' . $max . ' extra addresses.',
                'code' => 400,
            ];
        }
        $list[] = $email;
